...
function renderBusinessChecklist(item){
 ...
 document.querySelectorAll('[data-point-suggest]').forEach(b=>b.onclick=()=>{if(!hasProAccess()){openPlanPaywall('Sugestões personalizadas');return}const row=b.closest('.channelCheckItem');row.querySelector('.pointOutput').innerHTML=prototypeSuggestion(decodeURIComponent(b.dataset.title),decodeURIComponent(b.dataset.guidance));row.querySelector('.pointOutput').scrollIntoView({behavior:'smooth',block:'nearest'})});
 document.querySelectorAll('[data-point-mark]').forEach(b=>b.onclick=()=>{if(!hasProAccess()){openPlanPaywall('MARK.IA contextual');return}askMarkAboutPoint(decodeURIComponent(b.dataset.title),decodeURIComponent(b.dataset.guidance))});
...
function renderAnalysisDetail(id,sub=''){
 ...
 document.querySelectorAll('[data-analysis-suggest]').forEach((btn,i)=>btn.addEventListener('click',()=>{if(!hasProAccess()){openPlanPaywall('Sugestões personalizadas das análises');return}const row=btn.closest('.channelCheckItem');row.querySelector('.pointOutput').innerHTML=prototypeSuggestion(points[i],'Considere este ponto com base na realidade, objetivo e contexto atual da empresa.')}));
...
document.getElementById('planAvulsoBtn')?.addEventListener('click',()=>{route('home');setTimeout(()=>document.getElementById('estrategias')?.scrollIntoView({behavior:'smooth'}),80)});
...
function subscriptionCouponCode(plan){const id=plan==='premium'?'planPremiumCoupon':'planProCoupon';return String(document.getElementById(id)?.value||'').trim().toUpperCase()}
