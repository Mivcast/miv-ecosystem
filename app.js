const accessState={plan:'free',subscription:null,purchases:new Set(),ready:false};
...
let marketTrendItems=[],competitorInsightItems=[];
...
function fillSelectOptions(select,values,placeholder,value='',allowOther=false){
 if(!select)return;
 const current=value||select.value||'';
 select.innerHTML=`<option value="">${placeholder}</option>`+values.map(v=>`<option value="${v}">${v}</option>`).join('')+(allowOther?`<option value="${OTHER_NICHE_VALUE}">Outra opção</option>`:'');
 if(values.includes(current))select.value=current;
 else if(allowOther&&current)select.value=OTHER_NICHE_VALUE;
}
function otherInputFor(selectId){return document.getElementById(selectId.replace('Niche','OtherNiche').replace('Subniche','OtherSubniche'))}
function syncOtherInput(selectId,value=''){
 const select=document.getElementById(selectId),input=otherInputFor(selectId);if(!select||!input)return;
 const isOther=select.value===OTHER_NICHE_VALUE;
 input.hidden=!isOther;
 if(isOther&&value&&!input.value)input.value=value;
 if(!isOther)input.value='';
}
function selectedWithOther(selectId){
 const select=document.getElementById(selectId),input=otherInputFor(selectId);if(!select)return'';
 return select.value===OTHER_NICHE_VALUE?String(input?.value||'').trim():String(select.value||'').trim();
}
function renderCompanyNicheSelects(profile={}){
 ...
 const groupNames=groups.map(g=>g.name),mappedGroup=niches[currentNiche]?.group||currentNiche;
 fillSelectOptions(nicheSel,groupNames,'Selecione o nicho',mappedGroup,true);
 syncOtherInput('cpNiche',groupNames.includes(mappedGroup)?'':currentNiche);
 const group=groups.find(g=>g.name===nicheSel.value)||groups.find(g=>g.name===niches[currentNiche]?.group);
 const subValues=group?.subs||[];
 fillSelectOptions(subSel,subValues,'Selecione o subnicho',currentSub,true);
 syncOtherInput('cpSubniche',subValues.includes(currentSub)?'':currentSub);
 subSel.disabled=!group&&nicheSel.value!==OTHER_NICHE_VALUE;
}
function renderGenericNicheSelect(selectId,value=''){
 const values=Object.keys(niches).sort((a,b)=>a.localeCompare(b,'pt-BR'));
 fillSelectOptions(document.getElementById(selectId),values,'Selecione seu nicho ou subnicho',value,true);
 syncOtherInput(selectId,values.includes(value)?'':value);
}
function renderRegisterNicheSelect(){renderGenericNicheSelect('regNiche',selectedWithOther('regNiche'))}
function renderCalendarNicheSelect(value=''){renderGenericNicheSelect('calNiche',value)}
function bindNicheSelectWithOther(selectId,onChange){
 document.getElementById(selectId)?.addEventListener('change',e=>{syncOtherInput(selectId);if(onChange)onChange(e)});
 otherInputFor(selectId)?.addEventListener('input',()=>{if(onChange)onChange({target:document.getElementById(selectId)})});
}
...
const cardImageMap={
 ...
 'svc-logo':'assets/cards/svc-logo.jpg','svc-vetor':'assets/cards/svc-vetor.jpg','svc-manual':'assets/cards/svc-manual.jpg','svc-site':'assets/cards/svc-site.jpg','svc-loja':'assets/cards/svc-loja.jpg','svc-lp':'assets/cards/svc-lp.jpg','svc-market':'assets/cards/svc-market.jpg','svc-market40':'assets/cards/svc-market40.jpg','svc-google':'assets/cards/svc-google.jpg','svc-instagram':'assets/cards/svc-instagram.jpg','svc-15artes':'assets/cards/svc-instagram.jpg','svc-facebook':'assets/cards/svc-facebook.jpg','svc-whatsapp':'assets/cards/whatsapp.jpg','svc-link':'assets/cards/svc-lp.jpg','svc-linkedin':'assets/cards/svc-instagram.jpg','svc-midiakit':'assets/cards/svc-midiakit.jpg','svc-impressos':'assets/cards/svc-impressos.jpg','svc-arte':'assets/cards/svc-arte.jpg','svc-video':'assets/cards/videos.jpg','svc-institucional':'assets/cards/videos.jpg'
};
...
function mivcastCard(item){const saved=state.favorites.includes(item.id),short=String(item.desc||'').split('.').slice(0,2).join('. ').trim()+'.';return `<article class="mivcastServiceCard"><button class="heart ${saved?'saved':''}" data-fav="${item.id}">${saved?'♥':'♡'}</button><div class="mivcastServicePhoto"><img src="${imgFor(item)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove()"><span>${item.icon||'◆'}</span></div><small>${markEsc(item.tag||'MIVCAST')}</small><h3>${markEsc(item.title)}</h3><p>${markEsc(short)}</p><button class="open" data-open="${item.id}">Saiba mais →</button></article>`}
...
function renderShelf(id,ids){const el=document.getElementById(id);if(!el)return;const arr=ids.map(i=>getItem(i)).filter(Boolean);el.innerHTML=arr.map(id==='mivcastTrack'?mivcastCard:card).join('');bindCards()}
...
function saveMarketCache(){localStorage.setItem(MARKET_TRENDS_CACHE,JSON.stringify(marketTrendItems));localStorage.setItem(MARKET_COMPETITORS_CACHE,JSON.stringify(competitorInsightItems))}
function competitorLinkStore(){try{return JSON.parse(localStorage.getItem(MARKET_COMPETITOR_LINKS_CACHE)||'{}')||{}}catch(e){return{}}}
function saveCompetitorLinkStore(store){localStorage.setItem(MARKET_COMPETITOR_LINKS_CACHE,JSON.stringify(store||{}))}
function competitorChannelLinks(channel){const key=assetSlug(channel||'canal'),store=competitorLinkStore(),links=Array.isArray(store[key])?store[key]:[''];return links.length?links:['']}
function saveCompetitorChannelLinks(channel,links){const key=assetSlug(channel||'canal'),store=competitorLinkStore();store[key]=(links||['']).slice(0,competitorLimit());saveCompetitorLinkStore(store)}
function competitorLinksForChannel(channel){return competitorChannelLinks(channel).map(x=>String(x||'').trim()).filter(Boolean).slice(0,competitorLimit())}
function isLiveMarketItem(x){return x&&x.source&&x.source!=='MARK Radar'&&x.source!=='Análise orientativa'&&x.source!=='Buscando fontes reais'&&x.special!=='market-loading-placeholder'&&!String(x.title||'').includes('Completando tendências')}
function hydrateMarketCache(){localStorage.removeItem('mivMarketTrendItems');localStorage.removeItem('mivCompetitorInsightItems');try{marketTrendItems=(JSON.parse(localStorage.getItem(MARKET_TRENDS_CACHE)||'[]')||[]).filter(isLiveMarketItem)}catch(e){marketTrendItems=[]}try{competitorInsightItems=(JSON.parse(localStorage.getItem(MARKET_COMPETITORS_CACHE)||'[]')||[]).filter(isLiveMarketItem)}catch(e){competitorInsightItems=[]}if(!marketTrendItems.length)localStorage.removeItem(MARKET_TRENDS_CACHE);if(!competitorInsightItems.length)localStorage.removeItem(MARKET_COMPETITORS_CACHE)}
function marketStars(n){const qty=Math.max(0,Math.min(5,Math.round(Number(n)||0)));return `${'★'.repeat(qty)}${'☆'.repeat(5-qty)}`}
function marketEmptyCard(type,message,detail,buttonLabel,buttonId){
 return `<article class="market-card market-empty"><div class="market-card-top"><span class="market-stars">${type==='trends'?'★★★★★':'★★★☆☆'}</span><span class="meta-pill">${type==='trends'?'FONTES REAIS':'RADAR'}</span></div><div class="market-card-body"><h3>${markEsc(message)}</h3><p>${markEsc(detail)}</p><strong>Como funciona</strong><p>O MARK pesquisa na web, cruza com seu nicho e monta cards com título, resumo, estratégia de uso e link da fonte original.</p></div>${buttonId?`<button class="primary market-empty-action" data-click="${buttonId}">${markEsc(buttonLabel)}</button>`:''}</article>`;
}
function marketLoginCard(type){return `<article class="market-card market-empty market-auth-needed"><div class="market-card-top"><span class="market-stars">★★★★★</span><span class="meta-pill">LOGIN NECESSÁRIO</span></div><div class="market-card-body"><h3>Entre na sua conta para ${type==='trends'?'buscar tendências reais':'acompanhar concorrentes'}.</h3><p>Essa busca usa o perfil da sua empresa, seu nicho e o limite do seu plano. Por isso precisamos identificar sua conta antes de pesquisar.</p><strong>Próximo passo</strong><p>Faça login ou crie uma conta. Assim que entrar, o MARK volta para esta seção e continua a busca.</p></div><button class="primary market-empty-action" data-auth-market="${type}">Entrar na conta</button></article>`}
function marketLoadingCards(count,label){return Array.from({length:Math.min(Math.max(count,3),6)},(_,i)=>`<article class="market-card market-loading"><div class="market-card-top"><span class="market-stars">★★★★★</span><span class="meta-pill">PESQUISANDO</span></div><div class="market-card-body blurred"><h3>${i===0?'Buscando fontes reais...':'Cruzando notícia com seu nicho...'}</h3><p>O MARK está procurando ${label} recentes, verificando fonte e preparando uma estratégia prática para usar no seu conteúdo.</p><strong>Aguarde um instante</strong><p>Quando a resposta chegar, os cards antigos serão substituídos pelos resultados encontrados.</p></div></article>`).join('')}
...
function cleanRecentCompetitorSection(text=''){
 ...
 raw=raw.replace(/^Movimentação do mercado .* nos últimos dias:\s*/i,'').trim();
...
function mergeCompetitorRecent(base,recent){
 ...
 return {...base,...recent,id:base.id||recent.id,title:base.title||recent.title,channel:base.channel||recent.channel||recent.title,desc:`${baseDesc}\n\nMovimentação do mercado de ${state.profile.niche||'seu nicho'} nos últimos dias:\n${recentDesc}`,mark_strategy:[baseStrategy,recentStrategy].filter(Boolean).join('\n\n'),source:recent.source||base.source,source_url:recent.source_url||base.source_url,special:'competitor-insight'};
}
function competitorDescriptionHtml(text,item={}){
 const niche=state.profile.niche||'seu nicho',channel=item.channel||item.title||'canal';
 const normalized=String(text||'')
  .replace(/^Checklist\s+—\s+Posicionamento e Marketing de .+$/gmi,`Melhores estratégias para ${niche} performar bem no ${channel}`)
  .replace(/^Checklist\s+—\s+Movimentos recentes de .+$/gmi,`Movimentação do mercado de ${niche} nos últimos dias`)
  .replace(/^Movimentos intensificados nos últimos 30 dias:/gmi,`Movimentação do mercado de ${niche} nos últimos dias:`)
  .replace(/☐\s*/g,'• ');
 return markEsc(normalized).replace(/(^|<br>|\n)([^:\n<]{2,100}:)/g,(m,p,n)=>`${p}<b>${n}</b>`).replace(/\n/g,'<br>')
}
function marketCard(item,index,limit,lockedLabel){
 const saved=state.favorites.includes(item.id),locked=index>=limit,needed=index>=15?'Premium':'PRO',isCompetitor=item.special==='competitor-insight'||item.special==='competitor-placeholder',channelClass=isCompetitor?`competitor-channel-card channel-${assetSlug(item.channel||item.title)}`:'',bodyClass=locked&&isCompetitor?'':locked?'blurred':'',descHtml=isCompetitor?competitorDescriptionHtml(item.desc,item):markEsc(item.desc);
 ...
 const expand=!locked&&isCompetitor?`<button type="button" class="competitor-read-more" data-competitor-expand>Ver todo o card</button>`:'';
 return `<article class="market-card ${channelClass} ${isCompetitor&&!locked?'competitor-collapsed':''} ${locked?'locked':''} ${locked&&isCompetitor?'locked-channel':''}"><div class="market-card-top"><span class="market-stars" aria-label="${Number(item.importance)||0} de 5">${marketStars(item.importance)}</span><button class="heart ${saved?'saved':''}" ${locked?'data-scroll="planos"':`data-fav="${item.id}"`}>${locked?'🔒':saved?'♥':'♡'}</button></div><div class="market-card-body ${bodyClass}"><span class="meta-pill">${isCompetitor?'MOVIMENTO':item.cat}</span><h3>${markEsc(item.title)}</h3><p>${descHtml}</p><strong>${isCompetitor?'Nossa sugestão para você':'Como fazer igual ou melhor'}</strong><p>${markEsc(item.mark_strategy)}</p></div>${expand}${actions}${item.source_url&&!locked?`<a class="source-link" href="${markEsc(item.source_url)}" target="_blank" rel="noopener">Fonte: ${markEsc(item.source||sourceHost(item.source_url))}</a>`:`<small class="source-link">${markEsc(item.source||'Fonte do radar')}</small>`}${locked?`<div class="market-lock"><b>${lockedLabel||`Libere no ${needed}`}</b><span>Assine para acessar mais cards do seu mercado.</span><button class="primary" data-scroll="planos">Ver planos</button></div>`:''}</article>`;
}
...
function isReasonableCompetitorName(value){const v=String(value||'').trim();return v.length>=3&&v.length<=80&&!/https?:|www\.|\.com|\.com\.br|instagram\.com|facebook\.com|tiktok\.com|youtube\.com|[?!;]/i.test(v)&&!/\b(por que|saiba|entenda|confira|redes sociais|desinformação|ampliam|notícia|noticia|quando|onde|como|ipos?|esbanjam|brazil journal|medicina integrativa)\b/i.test(v)}
...
function setCompetitorStatus(message=''){const el=document.getElementById('competitorSearchStatus');if(el)el.textContent=message}
...
async function refreshMarketTrends(options={}){const {silent=false,keepExisting=false}=options||{},btn=document.getElementById('refreshTrends'),track=document.getElementById('trendsTrack');if(!mivUser){pendingAuthAction={type:'refreshTrends'};marketTrendItems=[];localStorage.removeItem(MARKET_TRENDS_CACHE);if(track){track.innerHTML=marketLoginCard('trends');bindMarketEmptyActions()}openAuth('login');toast('Entre na sua conta para buscar tendências reais.');return}if(!accessState.ready)await loadAccessFromSupabase();const limit=trendLimit();if(!silent)marketTrendAutoFillLimit=0;if(btn&&!silent){btn.disabled=true;btn.textContent='Buscando notícias reais...'}if(track&&!silent)track.innerHTML=marketLoadingCards(limit,'notícias e tendências');try{const token=await getAuthToken();const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:'trends',niche:state.profile.niche,limit})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar tendências agora.');const nextItems=(data.trends||[]).map((x,i)=>normalizeTrend(x,i)).filter(isLiveMarketItem).slice(0,limit);if(!nextItems.length)throw new Error('A busca veio sem cards publicáveis. Tente novamente; o MARK vai ampliar a pesquisa por notícias, estudos e movimentos recentes do nicho.');marketTrendItems=nextItems;saveMarketCache();if(track)track.scrollLeft=0;renderMarketTrends();if(!silent)toast(`${marketTrendItems.length} tendência(s) para seu nicho.`)}catch(e){console.warn('[Market trends]',e);if(!keepExisting){marketTrendItems=[];localStorage.removeItem(MARKET_TRENDS_CACHE);if(track){track.innerHTML=marketEmptyCard('trends','Não consegui montar tendências com fonte nesta tentativa.',e.message||'Tente atualizar novamente. A busca usa notícias, estudos e movimentos recentes do seu nicho, ampliando a janela quando necessário.','Tentar novamente','refreshTrends');bindMarketEmptyActions()}else renderMarketTrends()}if(!silent)toast('A busca de tendências não concluiu. Veja o card da seção.')}finally{if(btn&&!silent){btn.disabled=false;btn.textContent='Atualizar tendências'}}}
async function refreshCompetitors(suggest=false,channel='',link=''){
 const track=document.getElementById('competitorTrack'),suggestBtn=document.getElementById('suggestCompetitors'),refreshBtn=document.getElementById('refreshCompetitors'),btn=suggest?suggestBtn:refreshBtn;
 if(!mivUser){
  pendingAuthAction={type:'refreshCompetitors',suggest,channel,link};
  competitorInsightItems=[];
  localStorage.removeItem(MARKET_COMPETITORS_CACHE);
  if(track){track.innerHTML=marketLoginCard('competitors');bindMarketEmptyActions()}
 ...
 try{
  ...
  const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:suggest?'suggest_competitors':'competitors',niche:state.profile.niche,competitors:names,competitor_links:links,limit,channel})});
  const data=await r.json().catch(()=>({}));
  if(!r.ok)throw new Error(data.error||'Não foi possível buscar concorrentes agora.');
  if(suggest){
   const suggested=(Array.isArray(data.competitors)?data.competitors:[]).slice(0,limit);
   if(suggested.length){
    const input=document.getElementById('competitorNames');
    if(input)input.value=suggested.join('\n');
    saveCompetitorNames(suggested);
   }
   competitorInsightItems=[];
   localStorage.removeItem(MARKET_COMPETITORS_CACHE);
   renderCompetitorInsights();
   setCompetitorStatus(suggested.length?'Concorrentes sugeridos e salvos. Você pode remover, adicionar ou atualizar todos os canais.':'Não encontrei concorrentes reais e ativos com segurança. Digite nomes conhecidos para analisar.');
   toast(suggested.length?'Concorrentes sugeridos e salvos. Você pode remover, adicionar ou atualizar os canais.':'Não encontrei sugestões seguras; digite nomes para analisar.');
   return
  }
  const nextCards=(data.cards||[]).map((x,i)=>normalizeCompetitor(x,i)).filter(isLiveMarketItem).slice(0,7);
  if(!nextCards.length)throw new Error('A busca veio sem movimentos publicáveis. Tente novamente em alguns instantes.');
  if(channel){
  ...
  saveMarketCache();
  renderCompetitorInsights();
  setCompetitorStatus(channel?`${channel} atualizado com novidades e movimentos recentes.`:'Todos os meios foram atualizados com novidades e movimentos recentes.');
  toast(channel?`${channel} atualizado.`:'Novidades e movimentos atualizados.');
 }catch(e){
  console.warn('[Competitors]',e);
  setCompetitorStatus(e.message||'A busca de concorrentes não concluiu.');
  toast(e.message||'A busca de concorrentes não concluiu.');
...
function renderTracks(){document.getElementById('recTitle').textContent=`Mais úteis agora para ${state.profile.niche}`;renderShelf('recTrack',recommended());renderShelf('marketingTrack',[...bases.marketing,...bases.sales]);renderMarketTrends();renderCompetitorInsights();renderShelf('brandTrack',bases.brand);renderShelf('consultTrack',bases.consult);renderShelf('mivcastTrack',bases.mivcast);renderTools();renderLearn();try{renderMonthlyPlanControls()}catch(e){}}
...
function appendChecklistSection(map,id,name,points){
 const def=map?.[id];if(!def)return;
 def.sections.push({name,items:points.map((p,i)=>[`v2_${name.toLowerCase().replace(/\W+/g,'_')}_${i}`,p[0],p[1]])});
}
appendChecklistSection(marketingStrategyDefinitions,'calendario','Planejamento avançado',[
 ['Classificar cada data pela função da campanha','Venda, relacionamento, autoridade, conscientização, lançamento, fidelização ou institucional.'],
 ['Definir objetivo antes da campanha','Informe se quer movimento, lead, venda, recompra, reconhecimento ou outra meta.'],
 ['Indicar público específico da campanha','Evite campanha genérica quando o público muda por data, canal ou oferta.'],
 ['Escolher o canal mais apropriado','Instagram, WhatsApp, Google, loja, equipe, parceiros ou outro ponto de contato.'],
 ['Planejar preparação antecipada','Defina quantos dias antes a campanha precisa começar para não sair em cima da hora.'],
 ['Registrar resultado pós-campanha','Guarde o que funcionou para comparar datas e campanhas futuras.'],
 ['Manter banco de campanhas anteriores','Reaproveite criativos, mensagens e ofertas que provaram resultado.'],
 ['Separar calendário de lançamentos','Lançamentos precisam de cronograma próprio, não só datas comemorativas.'],
 ['Mapear datas de conscientização legítimas','Use causas e temas apenas quando houver conexão real com marca e nicho.'],
 ['Definir resultado esperado e métrica','Ex.: contatos, vendas, visitas, cliques, avaliações, recompra ou agendamentos.']
]);
appendChecklistSection(marketingStrategyDefinitions,'local','Pontos avançados',[
 ['Usar a vitrine como ferramenta comercial','A frente da empresa deve chamar atenção, explicar e provocar entrada ou contato.'],
 ['Medir fluxo de pessoas','Entenda dias, horários e pontos de maior circulação ao redor do negócio.'],
 ['Revisar estacionamento e acesso','Facilidade de chegada pode ser decisiva para negócios locais.'],
 ['Expor best-sellers estrategicamente','Produtos ou serviços mais fortes precisam aparecer melhor no ponto físico.'],
 ['Criar lançamentos e novidades no PDV','Dê motivo para voltar, perguntar e perceber movimento.'],
 ['Fazer eventos dentro da empresa','Experiências presenciais podem gerar conteúdo, relacionamento e vendas.'],
 ['Integrar loja, WhatsApp e Instagram','O cliente deve conseguir continuar a jornada no canal mais conveniente.'],
 ['Oferecer compra online e retirada','Quando aplicável, reduza atrito entre digital e físico.'],
 ['Permitir compra na loja e entrega','Facilite decisão para quem viu presencialmente, mas não quer carregar ou esperar.'],
 ['Usar QR Codes com propósito','Leve para catálogo, WhatsApp, avaliação, campanha ou conteúdo útil.'],
 ['Explorar marketing sensorial','Iluminação, música, aroma, textura e experimentação também vendem percepção.'],
 ['Capturar contatos no ponto físico','Crie motivo legítimo para continuar a conversa depois da visita.'],
 ['Mensurar origem dos clientes locais','Pergunte e registre como as pessoas chegaram até a empresa.']
]);
appendChecklistSection(marketingStrategyDefinitions,'equipe-marketing','Pontos avançados',[
 ['Avaliar clima e relacionamento interno','Ambiente interno ruim aparece no atendimento e na comunicação.'],
 ['Estimular cooperação entre setores','Marketing, vendas, atendimento e operação precisam trocar informações.'],
 ['Dar clareza de funções','Cada pessoa deve saber seu papel na experiência do cliente.'],
 ['Definir metas individuais e coletivas','Metas precisam orientar comportamento sem criar conflito artificial.'],
 ['Fazer treinamento recorrente','A equipe precisa revisar atendimento, objeções, produtos e promessas.'],
 ['Programar reciclagens','O padrão melhora quando volta para revisão, não só quando há problema.'],
 ['Criar política de comunicação','Deixe claro como falar com clientes em cada canal.'],
 ['Criar política de atendimento','Padronize acolhimento, prazos, retorno e passagem de responsabilidade.'],
 ['Definir tom de voz interno','A linguagem da equipe deve combinar com a marca.'],
 ['Tratar conflitos com processo','Problemas internos precisam de caminho claro de resolução.'],
 ['Coletar objeções relatadas pela equipe','Use dúvidas reais dos clientes para melhorar conteúdo, oferta e treinamento.'],
 ['Acompanhar indicadores por equipe','Meça atendimento, conversão, retorno, satisfação e gargalos.']
]);
appendChecklistSection(marketingStrategyDefinitions,'clientes-marketing','Pontos avançados',[
 ['Identificar ciclo de vida do cliente','Entenda quando ele compra, usa, renova, indica ou abandona.'],
 ['Definir frequência ideal de contato','Contato demais cansa; contato de menos deixa a marca desaparecer.'],
 ['Separar clientes por estágio','Novo, ativo, recorrente, VIP e inativo precisam de comunicação diferente.'],
 ['Identificar nível de consciência','Adapte mensagem para quem ainda não sabe do problema, compara opções ou já quer comprar.'],
 ['Criar comunicação por estágio','Boas-vindas, uso, recompra, reativação e indicação não devem receber a mesma mensagem.'],
 ['Pesquisar motivo de abandono','Entenda por que o cliente parou antes de oferecer desconto.'],
 ['Pesquisar motivo de recompra','Descubra o que realmente fez a pessoa voltar.'],
 ['Medir LTV quando possível','Valor de vida do cliente ajuda a decidir investimento e relacionamento.'],
 ['Reativar com contexto','Recupere clientes antigos com histórico, necessidade e motivo real, não promoção genérica.']
]);
appendChecklistSection(marketingStrategyDefinitions,'parcerias','Pontos avançados',[
 ['Avaliar qualidade do público','Seguidores ou alcance só importam se combinarem com o cliente ideal.'],
 ['Usar microcriadores para validação','Microcriadores podem testar mensagem, prova e aceitação em grupos menores.'],
 ['Convidar especialistas validadores','Profissionais reconhecidos podem fortalecer confiança.'],
 ['Mapear parceiros comerciais B2B','Empresas podem vender, indicar ou complementar sua solução.'],
 ['Criar afiliados ou indicadores quando aplicável','Formalize indicação quando fizer sentido financeiro e ético.'],
 ['Preparar material profissional para parceiros','Facilite como o parceiro apresenta sua empresa.'],
 ['Criar página ou kit de parceiro','Centralize oferta, provas, orientações e materiais.'],
 ['Separar parceria para conteúdo, evento, produto e venda','Cada tipo de parceria precisa de entrega e métrica próprias.']
]);
appendChecklistSection(marketingStrategyDefinitions,'identidade-marketing','Pontos avançados',[
 ['Definir direção fotográfica','Fotos precisam combinar com percepção desejada, público e canal.'],
 ['Produzir fotografia de produto','Imagem boa aumenta clareza, confiança e desejo.'],
 ['Usar fotografia lifestyle quando fizer sentido','Mostre produto, serviço ou transformação no contexto de uso.'],
 ['Definir estilo de vídeo','Cortes, ritmo, cenário e linguagem devem reforçar a marca.'],
 ['Organizar iconografia','Ícones e sinais visuais ajudam a explicar sem poluir.'],
 ['Aplicar identidade na ambientação','Ambiente físico também deve parecer a mesma marca.'],
 ['Planejar vitrine e preço visual','Preço, ofertas e destaques precisam de apresentação profissional.'],
 ['Criar materiais para parceiros','Parceiros precisam receber peças coerentes e fáceis de usar.'],
 ['Padronizar apresentação comercial','Propostas e apresentações devem carregar a mesma percepção.'],
 ['Criar padrão de anúncios','Anúncios precisam vender sem fugir da identidade.'],
 ['Criar materiais institucionais','Apresentações, documentos e materiais internos também comunicam marca.']
]);
appendChecklistSection(marketingStrategyDefinitions,'brindes','Pontos avançados',[
 ['Criar brinde para lançamento','Use bônus para ativar curiosidade e primeira experiência.'],
 ['Elevar a experiência com brinde','O brinde deve aumentar percepção, não parecer sobra.'],
 ['Oferecer bônus digital','Guias, checklists e aulas podem complementar produtos e serviços.'],
 ['Criar benefício VIP','Clientes mais valiosos podem receber experiências especiais.'],
 ['Usar benefício progressivo','Reconheça frequência, ticket ou indicação de forma sustentável.'],
 ['Gerar surpresa pós-compra','Surpresas pequenas podem aumentar lembrança e indicação.'],
 ['Usar amostras ou testes quando aplicável','Experimentação reduz risco percebido.'],
 ['Medir impacto em recompra e indicação','Brinde bom precisa gerar comportamento, não só custo.']
]);
appendChecklistSection(businessChecklistDefinitions,'branding-fisico','Pontos avançados',[
 ['Separar fachada e vitrine','A fachada identifica; a vitrine provoca interesse e venda.'],
 ['Revisar circulação e disposição','O caminho do cliente no ambiente influencia decisão.'],
 ['Trabalhar os cinco sentidos','Visão, som, aroma, toque e experiência podem reforçar posicionamento.'],
 ['Pensar antes, durante e depois da compra','Branding físico começa antes da entrada e continua após a saída.'],
 ['Conectar ticket e ambiente','Preço, experiência e percepção precisam conversar.'],
 ['Expor produtos estratégicos','O ambiente deve ajudar a vender o que mais importa.'],
 ['Tratar o ambiente como vendedor','Espaço físico precisa orientar, explicar e gerar confiança.'],
 ['Conectar QR Codes e digital','Leve o cliente para WhatsApp, catálogo, avaliações e campanhas.'],
 ['Padronizar atendimento presencial','A experiência humana é parte central da marca.']
]);
appendChecklistSection(businessChecklistDefinitions,'branding-digital','Pontos avançados',[
 ['Aprofundar tom de voz','Defina como a marca fala, responde, orienta e vende.'],
 ['Explicitar personalidade da marca','A comunicação precisa ter traços reconhecíveis.'],
 ['Definir promessa central','Deixe claro o que a marca se compromete a entregar.'],
 ['Ter mensagem principal','A ideia central deve aparecer nos principais canais.'],
 ['Garantir consistência entre promessa e entrega','Não prometa uma experiência que o negócio não sustenta.'],
 ['Mostrar fundador, equipe ou especialistas','Pessoas reais aumentam confiança quando coerentes com o nicho.'],
 ['Criar biblioteca de provas','Avaliações, casos, resultados e bastidores legítimos precisam estar organizados.'],
 ['Cuidar da reputação digital','Comentários, avaliações e buscas precisam ser acompanhados.'],
 ['Usar mídia/imprensa quando existir','Menções externas podem reforçar autoridade.'],
 ['Demonstrar autoridade técnica','Ensine, explique e prove conhecimento sem exagero.'],
 ['Manter coerência digital completa','Site, redes, WhatsApp, anúncios e Google devem parecer parte da mesma marca.']
]);
appendChecklistSection(businessChecklistDefinitions,'identidade','Pontos avançados',[
 ['Definir personalidade visual','A identidade deve transmitir sensações compatíveis com a marca.'],
 ['Mapear percepção desejada','Clareza, sofisticação, proximidade, técnica ou velocidade mudam a estética.'],
 ['Adequar ao público','Visual bonito mas desalinhado com o cliente perde força.'],
 ['Adequar ao posicionamento','Identidade deve sustentar preço, promessa e categoria.'],
 ['Aplicar em embalagem','Produto físico precisa continuar a marca na mão do cliente.'],
 ['Definir fotografia e audiovisual','Imagem e vídeo também fazem parte da identidade.'],
 ['Criar templates comerciais','Orçamentos, propostas e apresentações precisam de padrão.'],
 ['Criar materiais internos','Equipe deve usar materiais coerentes no dia a dia.'],
 ['Manter consistência em ambientes físicos','A loja, clínica ou escritório deve parecer a mesma marca.']
]);
appendChecklistSection(businessChecklistDefinitions,'diferenciacao','Pontos avançados',[
 ['Separar atributo comum de diferencial real','Nem todo benefício básico pode ser vendido como diferencial.'],
 ['Comparar o que cada concorrente domina','Entenda território, força e promessa de cada alternativa.'],
 ['Identificar território livre','Procure uma posição que ninguém ocupa claramente.'],
 ['Aprender com concorrentes sem copiar','Observe processos, canais e provas que funcionam.'],
 ['Definir o que não copiar','Proteja autenticidade e posicionamento.'],
 ['Transformar diferencial em processo','Diferencial forte aparece na entrega, não só na frase.'],
 ['Criar razões para acreditar','Provas, método, bastidores e resultados sustentam a promessa.'],
 ['Testar posicionamento','Valide a percepção antes de tratar como verdade definitiva.']
]);
appendChecklistSection(businessChecklistDefinitions,'ticket','Pontos avançados',[
 ['Oferecer segunda unidade','Quando fizer sentido, facilite levar mais de uma unidade.'],
 ['Criar assinatura ou recorrência','Transforme reposição ou acompanhamento em receita previsível.'],
 ['Adicionar serviço complementar','Serviço pode elevar valor percebido e resultado.'],
 ['Montar pacotes','Pacotes ajudam a organizar escolha e ticket.'],
 ['Usar venda cruzada','Recomende itens que melhoram a experiência principal.'],
 ['Criar benefício progressivo','Incentive ticket maior sem destruir margem.'],
...
appendChecklistSection(businessChecklistDefinitions,'followup','Pontos avançados',[
 ...
 ['Registrar motivo de não fechamento','A causa da perda orienta melhoria de oferta e atendimento.'],
 ['Criar retorno automático','Lembretes reduzem oportunidades esquecidas.'],
 ['Separar reativação de follow-up','Reativar cliente antigo é diferente de acompanhar lead recente.'],
 ['Combinar remarketing e follow-up','Anúncios e conversa podem trabalhar juntos.'],
 ['Usar status comercial','Organize em novo, aguardando, negociando, perdido e vendido.'],
 ['Definir próxima ação obrigatória','Toda oportunidade precisa ter um próximo passo.'],
 ['Definir responsável','Sem dono, o lead some.'],
 ['Medir valor potencial perdido','Ajuda a priorizar recuperação.'],
 ['Medir taxa de recuperação','Acompanhe quanto o follow-up realmente salva.']
]);
appendChecklistSection(businessChecklistDefinitions,'fidelizacao','Pontos avançados',[
 ['Mapear ciclo de renovação','Saiba quando o cliente deveria voltar.'],
 ['Criar lembrete automático','Automação ajuda a aparecer no momento certo.'],
 ['Montar clube de benefícios','Benefícios organizados aumentam recorrência.'],
 ['Oferecer assinatura quando fizer sentido','Recorrência reduz dependência de venda avulsa.'],
 ['Criar recompensa progressiva','Valorize frequência, ticket e indicação.'],
 ['Identificar cliente VIP','Clientes melhores merecem experiência melhor.'],
 ['Medir NPS ou satisfação','Pergunte antes de perder o cliente.'],
 ['Registrar motivo de recompra','Descubra o que gera retorno.'],
 ['Registrar motivo da perda','Use abandono como dado para melhorar.'],
 ['Medir valor do cliente no tempo','LTV orienta investimento em relacionamento.']
]);
appendChecklistSection(businessChecklistDefinitions,'whatsapp','Pontos avançados',[
 ['Identificar tipo de cliente','Novo, recorrente, indicação, orçamento ou suporte mudam a conversa.'],
 ['Identificar nível de consciência','A abordagem muda conforme maturidade do comprador.'],
 ['Registrar origem','Saiba se veio de Instagram, Google, indicação, campanha ou loja.'],
 ['Diagnosticar necessidade','Entenda o problema antes de oferecer solução.'],
 ['Criar roteiro por produto ou serviço','Scripts precisam refletir a oferta específica.'],
 ['Preparar respostas às objeções','Preço, prazo, confiança e comparação devem ter resposta clara.'],
 ['Manter biblioteca de respostas','Organize mensagens recorrentes e atualize com base nas dúvidas reais.'],
 ['Criar script de follow-up','Acompanhe sem parecer insistente.'],
 ['Criar script de reativação','Volte com contexto e motivo.'],
 ['Criar script de pós-venda','Confirme satisfação, uso, suporte e próximos passos.'],
 ['Criar script de indicação','Facilite o cliente indicar quando estiver satisfeito.'],
 ['Criar script de renovação','Antecipe recompra ou continuidade.'],
 ['Definir passagem para humano','Saiba quando automação ou roteiro deve virar atendimento humano.']
]);
appendChecklistSection(businessChecklistDefinitions,'oferta','Pontos avançados',[
 ['Calcular custo e margem mínima','Oferta sustentável começa pela margem.'],
 ['Comparar preço e valor com mercado','Entenda se o preço está caro, barato ou mal explicado.'],
 ['Segmentar por público','Públicos diferentes podem precisar de versões diferentes da oferta.'],
 ['Criar pacote, plano ou assinatura','Organize escolha e recorrência quando fizer sentido.'],
 ['Adicionar cross-sell e upsell','Mostre complementos e upgrades úteis.'],
 ['Explicar formas de pagamento','Pix, cartão, parcelamento e condições devem estar claros.'],
 ['Explicar entrega e prazo','Reduza dúvidas que travam decisão.'],
 ['Explicar política de troca, garantia ou suporte','Diminua risco percebido.'],
 ['Criar oferta de entrada, principal e premium','Níveis de oferta ajudam diferentes momentos de compra.'],
 ['Usar desconto só quando sustentável','Promoção precisa ter objetivo e margem.']
]);
...
function prototypeSuggestion(title,guidance){
  ...
  return `<div class="pointSuggestion"><span class="eyebrow">SUGESTÃO PERSONALIZADA</span><strong>${title}</strong><p>${guidance}</p><p><b>Considerando seu contexto:</b> ${ctx}</p><p>Use este ponto como prioridade inicial e peça ao MARK para transformar em plano quando quiser aprofundar.</p></div>`;
}
async function renderPointSuggestion(row,title,guidance,scope='ponto'){
 const out=row?.querySelector('.pointOutput');if(!out)return;
 out.innerHTML='<div class="pointSuggestion"><span class="eyebrow">SUGESTÃO PERSONALIZADA</span><p>MARK está analisando seu contexto...</p></div>';out.scrollIntoView({behavior:'smooth',block:'nearest'});
 try{
  if(!mivUser||!mivSupabase)throw new Error('Entre na sua conta para gerar a sugestão.');
  const {data:sessionData,error}=await mivSupabase.auth.getSession();if(error)throw error;const token=sessionData?.session?.access_token;if(!token)throw new Error('Sessão expirada. Entre novamente.');
  const current=state.current||null,question=`Gere uma sugestão curta, prática e personalizada para este ${scope}: "${title}". Orientação do ponto: ${guidance}. Use o Perfil da Empresa, o nicho atual e a metodologia cadastrada. Responda com diagnóstico breve, uma ação prioritária e o primeiro passo.`;
  const context={route:state.route||'detail',item_id:current?.id||'',title:current?.title||title,point:title};
  const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({question,history:[],context})});
  const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível gerar a sugestão.');
  out.innerHTML=`<div class="pointSuggestion"><span class="eyebrow">SUGESTÃO PERSONALIZADA</span><strong>${markEsc(title)}</strong><p>${markEsc(data.answer||'').replace(/\n/g,'<br>')}</p></div>`;
 }catch(err){
  out.innerHTML=prototypeSuggestion(title,guidance);
  toast(err.message||'Sugestão local gerada.');
 }
}
function askMarkAboutPoint(title,guidance){
 ...
 if(input){input.value=`Quero ajuda com este ponto: "${title}". Contexto do ponto: ${guidance}`;input.focus()}
...
function addChannelPoints(id,points){
 const ch=channelDefinitions.find(x=>x.id===id);if(!ch)return;
 ch.items.push(...points.map((p,i)=>[`v2_${id}_${i}`,p[0],p[1]]));
}
addChannelPoints('instagram',[
 ['Conteúdo separado por função','Organize educação, autoridade, prova, relacionamento e venda.'],
 ['Reels e vídeos são avaliados','O canal não deve ser medido apenas pela bio e feed estático.'],
 ['Bastidores e presença humana aparecem quando ajudam','Pessoas reais aumentam confiança em muitos nichos.'],
 ['Fundador, equipe ou especialistas aparecem quando faz sentido','Autoridade humana pode reduzir insegurança e aproximar a marca.'],
 ['Dúvidas recorrentes viram conteúdo','FAQ do atendimento deve alimentar posts, stories, reels e destaques.'],
 ['Resultados do canal são acompanhados','Observe alcance, clique, conversa, orçamento e venda.'],
 ['Instagram leva ao canal de conversão','A pessoa precisa entender claramente como comprar, agendar ou pedir orçamento.']
]);
addChannelPoints('whatsapp',[
 ['Biblioteca de respostas está organizada','Padronize respostas por dúvida, produto, objeção e estágio.'],
 ['Lead é qualificado no início','Antes de preço, entenda perfil, necessidade, urgência e contexto.'],
 ['Necessidade é identificada antes da oferta','Apresente a solução certa em vez de mandar catálogo genérico.'],
 ['Existe transferência para atendimento humano','Defina quando a conversa precisa sair do roteiro.'],
 ['Contatos são organizados por estágio comercial','Novo lead, orçamento, negociação, cliente, pós-venda e reativação.'],
 ['Renovação ou recompra são lembradas','Quando aplicável, antecipe a próxima necessidade do cliente.'],
 ['Clientes inativos têm reativação planejada','Volte com contexto, não só com promoção.'],
 ['Origem do lead é registrada','Saiba se veio de Instagram, Google, indicação, site, loja ou anúncio.'],
 ['Conversa, orçamento e venda são medidos','Acompanhe a taxa real de conversão do atendimento.']
]);
addChannelPoints('google',[
 ['Produtos e serviços estão cadastrados','Ajude o público e o Google a entenderem o que você vende.'],
 ['Perguntas e respostas são usadas','Antecipe dúvidas frequentes no próprio perfil.'],
 ['Palavras-chave locais aparecem naturalmente','Serviço, nicho, bairro, cidade e região precisam estar claros.'],
 ['Avaliações recebem resposta estratégica','Responda com cuidado, prova de atendimento e contexto.'],
 ['Fotos têm qualidade e frequência','Fotos reais e atuais aumentam confiança e descoberta.'],
 ['Concorrentes locais são comparados','Veja quem aparece melhor e por quê.'],
 ['Ligações, rotas, cliques e contatos são acompanhados','Mensure o papel do Google no movimento e nas oportunidades.']
]);
addChannelPoints('site',[
 ['Há páginas por grande necessidade do cliente','Páginas específicas convertem melhor do que uma página genérica.'],
 ['Produtos são organizados por necessidade','Ajude o cliente a escolher pelo problema que quer resolver.'],
 ['FAQ está presente','Responda dúvidas que travam compra ou contato.'],
 ['Vídeos ajudam a explicar','Vídeo pode aumentar confiança e entendimento.'],
 ['Depoimentos reais ou provas legítimas aparecem','Use prova social apenas quando verdadeira e autorizada.'],
 ['Garantia e políticas estão claras','Reduza insegurança sobre troca, entrega, suporte e prazos.'],
 ['Preço, Pix e parcelamento estão claros','Evite surpresa no fim da jornada.'],
 ['Prazo e entrega são explicados','Clientes precisam saber o que acontece depois da compra.'],
 ['Produtos relacionados e kits são sugeridos','Facilite segunda compra, cross-sell e composição de oferta.'],
 ['Existe captura de lead para quem não compra','Nem todo visitante está pronto para comprar agora.'],
 ['Abandono e remarketing são considerados','Crie caminhos para recuperar interesse.'],
 ['SEO individual das páginas foi planejado','Cada página relevante precisa poder ser encontrada.']
]);
addChannelPoints('facebook',[
 ['Avaliações e provas estão visíveis','Use prova real para aumentar confiança.'],
 ['Comunidades ou grupos são considerados','Em alguns nichos, grupos podem gerar relacionamento e demanda.'],
 ['Remarketing Meta está previsto','Facebook pode apoiar anúncios para quem já interagiu.'],
 ['Instagram, Facebook e WhatsApp estão integrados','A jornada comercial precisa ficar conectada.']
]);
addChannelPoints('tiktok',[
 ['Tendências compatíveis são filtradas','Use tendências apenas quando combinam com o nicho e a marca.'],
 ['Vídeos educativos são planejados','Ensine algo útil para gerar descoberta e autoridade.'],
 ['Prova social aparece no formato certo','Resultados e bastidores podem reforçar confiança.'],
 ['Bastidores são explorados','Mostre processo, rotina, equipe e transformação.'],
 ['Perguntas frequentes viram vídeos','Dúvidas reais costumam gerar bons conteúdos.'],
 ['TikTok Analytics é acompanhado','Meça retenção, visualizações, perfil e cliques.'],
 ['Retenção dos vídeos é analisada','O início e o ritmo precisam segurar atenção.']
]);
addChannelPoints('physical',[
 ['Vitrine é planejada','A vitrine deve vender atenção, categoria, novidade e desejo.'],
 ['Circulação e layout são revisados','O espaço precisa facilitar descoberta, escolha e atendimento.'],
 ['Zonas quentes são aproveitadas','Coloque itens estratégicos onde o cliente realmente olha e passa.'],
 ['Exposição dos produtos é intencional','Destaques, best-sellers e lançamentos precisam ter lugar.'],
 ['Planograma simples existe quando aplicável','Organize disposição para reduzir bagunça e melhorar venda.'],
 ['Iluminação reforça percepção','Luz comunica cuidado, valor e conforto.'],
 ['Música e aroma são usados com critério','Sensorial ajuda quando combina com público e ambiente.'],
 ['Textura, experimentação ou degustação são consideradas','Teste e contato reduzem risco quando aplicável.'],
 ['Estacionamento, acesso e localização são comunicados','Facilite chegada e reduza dúvidas práticas.'],
 ['QR Code conecta físico e digital','Use para catálogo, WhatsApp, avaliação, conteúdo ou campanha.']
]);
addChannelPoints('relationships',[
 ['Parceiros B2B são mapeados','Empresas complementares podem gerar venda e indicação.'],
 ['Prescritores e indicadores são considerados','Profissionais que influenciam decisão podem fortalecer aquisição.'],
 ['Influenciadores e microinfluenciadores são avaliados','Priorize coerência e qualidade do público.'],
 ['Clientes embaixadores são identificados','Clientes satisfeitos podem indicar, provar e defender a marca.'],
 ['Fornecedores estratégicos são considerados','Alguns fornecedores ajudam com informação, co-marketing ou visibilidade.'],
 ['Associações e comunidades são mapeadas','Grupos certos podem abrir relacionamento e autoridade.'],
 ['Programa formal de indicação existe quando faz sentido','Deixe regra, benefício e rastreamento claros.'],
...
function bindChannelChecklist(){
 ...
 document.querySelectorAll('[data-point-suggest]').forEach(b=>b.onclick=()=>{
   if(!hasProAccess()){openPlanPaywall('Sugestões personalizadas');return}
   const row=b.closest('.channelCheckItem'),title=decodeURIComponent(b.dataset.title),guidance=decodeURIComponent(b.dataset.guidance);
    renderPointSuggestion(row,title,guidance,'ponto do checklist');
 });
...
function renderCommercialPage(item,type){
 ...
 document.querySelector('[data-talk-commercial]')?.addEventListener('click',()=>{const input=document.getElementById('markInput');document.getElementById('mark')?.classList.add('open');if(input){input.value=`Quero tirar dúvidas antes de contratar: ${item.title}`;input.focus()}});
}
...
function showToolPreview(item){
 state.current=item;document.getElementById('detailVisual').style.backgroundImage=`url('${item.img}')`;document.getElementById('detailCat').textContent='FERRAMENTA EM DESENVOLVIMENTO';document.getElementById('detailTitle').textContent=item.title;document.getElementById('detailDesc').textContent=item.desc;document.getElementById('favBtn').textContent=state.favorites.includes(item.id)?'♥ Salvo':'♡ Salvar';document.getElementById('useBtn').textContent='Quero ser avisado';document.getElementById('useBtn').onclick=()=>toast('Interesse registrado. Quando o recurso estiver pronto, ele poderá aparecer na sua Central.');
 const benefits={'markia-app':['Consultor inteligente com contexto da empresa','Sugestões dentro dos cards, análises e relatórios','Histórico de conversas e decisões','Apoio contínuo para priorizar próximos passos'],'agendamentos':['Agenda de atendimentos e horários','Organização de clientes e retornos','Lembretes e visão da rotina','Integração futura com WhatsApp e Central'],'financeiro':['Entradas, saídas e contas em um só lugar','Visão simples de fluxo de caixa','Alertas para obrigações e recorrências','Conexão futura com metas e relatórios'],'orcamentos':['Criação de propostas e orçamentos','Acompanhamento de status comercial','Histórico por cliente','Base para follow-up e fechamento']}[item.id]||['Recurso conectado à Central','Uso integrado aos cards','Histórico e evolução','Apoio do MARK contextual'];
 document.getElementById('detailBody').innerHTML=`<span class="eyebrow">SAIBA MAIS</span><h2>${markEsc(item.title)}</h2><p>${markEsc(item.desc)}</p><section class="commercialSection"><h3>O que este recurso deve resolver</h3><div class="benefitChecklist">${benefits.map(x=>`<div>✓ ${markEsc(x)}</div>`).join('')}</div></section><section class="commercialSection"><h3>Status</h3><p>Recurso em fase de preparação. Ele aparece na vitrine para o usuário entender o caminho do ecossistema e salvar interesse, sem prometer disponibilidade imediata.</p></section><div class="commercialCTAs"><button class="primary" data-tool-interest>Quero ser avisado</button><button class="outline" data-tool-mark>Perguntar ao MARK</button></div>`;
 document.querySelector('[data-tool-interest]')?.addEventListener('click',()=>toast('Interesse registrado.'));
 document.querySelector('[data-tool-mark]')?.addEventListener('click',()=>askMarkAboutPoint(item.title,'Quero entender como esta ferramenta pode ajudar meu negócio quando estiver disponível.'));
 document.getElementById('related').innerHTML=`<div class="relatedItem"><strong>Salvar na Central</strong><span>Use o coração para guardar esta ferramenta e acompanhar depois.</span></div><div class="relatedItem"><strong>Contexto conectado</strong><span>Quando ficar pronta, poderá usar dados do Perfil da Empresa e relatórios.</span></div>`;addHistory(item);route('detail')
}

function openItem(id){const sp=getItem(id);if(!sp)return;const commercial=['consult-sale','service-sale','notify'].includes(sp.special);if(!commercial&&!hasItemAccess(sp)){openPaywall(sp);return}if(sp?.special==='calendar'){openCalendar();return}if(sp?.special==='channels'){showChannelsDetail(sp);return}if(sp?.special==='marketing-checklist'){showMarketingChecklist(sp);return}if(sp?.special==='business-checklist'){showBusinessChecklist(sp);return}if(sp?.special==='consult-sale'){showCommercialDetail(sp,'consult');return}if(sp?.special==='service-sale'){showCommercialDetail(sp,'service');return}if(sp?.special==='notify'){showToolPreview(sp);return}const item=getItem(id);if(!item)return;state.current=item;showDetail(item)}
function showDetail(item){state.current=item;if(item._learning){document.getElementById('useBtn').textContent=item.learningVideoId?'Assistir no sistema':item.learningExternal?'Abrir material':'Marcar como aplicado';document.getElementById('useBtn').onclick=()=>{if(item.learningVideoId)openLearningVideo(item);else if(item.learningExternal)window.open(item.learningExternal,'_blank','noopener');else toast('Conteúdo marcado como aplicado no seu histórico.');};document.getElementById('detailVisual').style.backgroundImage=`url('${item.img}')`;document.getElementById('detailCat').textContent=`APRENDA & APLIQUE · ${String(item.format||'CONTEÚDO').toUpperCase()}`;document.getElementById('detailTitle').textContent=item.title;document.getElementById('detailDesc').textContent=item.desc;document.getElementById('favBtn').textContent=state.favorites.includes(item.id)?'♥ Salvo':'♡ Salvar';document.getElementById('detailBody').innerHTML=`<span class="eyebrow">${item.learningArea||'APRENDIZADO'}</span><h2>Aplicação prática</h2><div class="learningBody">${String(item.learningBody||item.desc||'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</div>${item.learningVideoId?`<button class="primary learningExternal" data-watch-learning>Assistir aula no sistema</button>`:item.learningExternal?`<a class="outline learningExternal" href="${item.learningExternal}" target="_blank" rel="noopener">Abrir material externo →</a>`:''}<h2>Use o MARK.IA</h2><p>Peça ao MARK para adaptar este conteúdo ao contexto da sua empresa e transformar o aprendizado em próximos passos.</p>`;document.querySelector('[data-watch-learning]')?.addEventListener('click',()=>openLearningVideo(item));document.getElementById('related').innerHTML=items.filter(x=>x._learning&&x.id!==item.id&&(x.learningArea===item.learningArea)).slice(0,5).map(x=>`<div class="relatedItem" data-related="${x.id}"><strong>${x.title}</strong><span>${x.format} · ${x.access}</span></div>`).join('');document.querySelectorAll('[data-related]').forEach(x=>x.onclick=()=>openItem(x.dataset.related));addHistory(item);route('detail');return;}document.getElementById('useBtn').textContent='Usar agora';document.getElementById('useBtn').onclick=()=>toast('Resultado salvo na sua Central.');document.getElementById('detailVisual').style.backgroundImage=`url('${item.img}')`;document.getElementById('detailCat').textContent=`${item.cat.toUpperCase()} · ${item.format.toUpperCase()}`;document.getElementById('detailTitle').textContent=item.title;document.getElementById('detailDesc').textContent=item.desc;document.getElementById('favBtn').textContent=state.favorites.includes(item.id)?'♥ Salvo':'♡ Salvar';document.getElementById('detailBody').innerHTML=`<span class="eyebrow">APLICAÇÃO DO CARD</span><h2>Conteúdo prático e conectado ao seu contexto</h2><p>Esta área combina orientação objetiva, exemplos específicos do nicho, checklist, ferramenta interativa ou resultado salvável conforme a finalidade do card.</p><h2>Aplicação sugerida</h2><ul><li>Entenda o objetivo desta solução.</li><li>Adapte ao contexto de <strong>${state.profile.niche}</strong>.</li><li>Escolha uma única ação prioritária.</li><li>Salve o resultado na sua Central.</li><li>Peça ao MARK para revisar ou transformar em plano.</li></ul><h2>MARK nesta página</h2><p>O MARK sabe que você está vendo <strong>${item.title}</strong> e pode responder considerando seu nicho atual.</p>`;const rel=items.filter(x=>x.id!==item.id&&(x.cat===item.cat||x.tag===item.tag)).slice(0,5);document.getElementById('related').innerHTML=rel.map(x=>`<div class="relatedItem" data-related="${x.id}"><strong>${x.title}</strong><span>${x.format} · ${x.access}</span></div>`).join('');document.querySelectorAll('[data-related]').forEach(x=>x.onclick=()=>openItem(x.dataset.related));addHistory(item);route('detail')}
...
function openPaywall(item){state.current=item;if(!mivUser&&item?.id)pendingAuthAction={type:'paywall',itemId:item.id};document.getElementById('payTitle').textContent=item.title;document.getElementById('singlePrice').textContent=item.price||'Plano Pro';const cc=document.getElementById('couponCode');if(cc)cc.value='';const cf=document.getElementById('couponFeedback');if(cf){cf.style.display='none';cf.textContent=''};const bs=document.getElementById('buySingle');if(bs)bs.childNodes[0].textContent='Comprar este item ';document.getElementById('paywall').classList.add('show');document.getElementById('overlay').classList.add('show')}

async function applyCouponPreview(){
 const item=state.current,code=(document.getElementById('couponCode')?.value||'').trim();const feedback=document.getElementById('couponFeedback'),btn=document.getElementById('applyCoupon');
 if(!code){if(feedback){feedback.style.display='block';feedback.textContent='Digite um cupom.'}return}if(!mivUser||!mivSupabase){pendingAuthAction={type:'paywall',itemId:item?.id};closePaywall();openAuth('login');toast('Entre na sua conta para aplicar o cupom.');return}
 try{if(btn){btn.disabled=true;btn.textContent='Validando...'}const {data:sessionData,error}=await mivSupabase.auth.getSession();if(error)throw error;const token=sessionData?.session?.access_token;if(!token)throw new Error('Sessão expirada. Entre novamente.');const r=await fetch('/api/validate-coupon',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({item_id:normalizeAccessItemId(item.id),coupon_code:code})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Cupom inválido.');const money=v=>Number(v).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});document.getElementById('singlePrice').textContent=money(data.amount);if(feedback){feedback.style.display='block';feedback.innerHTML=`<strong style="color:#18864b">Cupom ${data.code} aplicado ✓</strong><br><span style="text-decoration:line-through">${money(data.original)}</span> · desconto ${money(data.discount)} · <strong>Total ${money(data.amount)}</strong>`}const buy=document.getElementById('buySingle');if(buy)buy.childNodes[0].textContent=data.free_unlock?'Liberar com cupom ':'Comprar por ';}
 catch(err){if(feedback){feedback.style.display='block';feedback.innerHTML=`<strong style="color:#b42318">${err.message||'Cupom inválido.'}</strong>`}document.getElementById('singlePrice').textContent=item.price||'Plano Pro';}
 finally{if(btn){btn.disabled=false;btn.textContent='Aplicar'}}
}
async function startMercadoPagoSinglePurchase(){
 const item=state.current;
 if(!item)return;
 if(!mivUser||!mivSupabase){pendingAuthAction={type:'paywall',itemId:item?.id};closePaywall();openAuth('login');toast('Entre na sua conta para comprar.');return}
 const btn=document.getElementById('buySingle');const old=btn?.innerHTML;
 try{
  if(btn){btn.disabled=true;btn.textContent='Abrindo Mercado Pago...'}
  const {data:sessionData,error:sessionError}=await mivSupabase.auth.getSession();
  if(sessionError)throw sessionError;const token=sessionData?.session?.access_token;if(!token)throw new Error('Sessão expirada. Entre novamente.');
  const r=await fetch('/api/create-mercadopago-preference',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({item_id:normalizeAccessItemId(item.id),coupon_code:(document.getElementById('couponCode')?.value||'').trim()})});
  const data=await r.json().catch(()=>({}));
  if(!r.ok)throw new Error(data.error||'Não foi possível abrir o pagamento.');
  if(data.free_unlock){closePaywall();toast('Cupom de 100% aplicado. Item liberado!');await loadAccessState();renderAll();return}if(!data.checkout_url)throw new Error('URL do checkout não recebida.');
  window.location.href=data.checkout_url;
 }catch(err){console.error('[MIV checkout]',err);toast(err.message||'Não foi possível abrir o Mercado Pago.');if(btn){btn.disabled=false;btn.innerHTML=old||'Comprar este item'}}
}
...
const analysisSubDescriptions={
 'Estrutura Ideal do Negócio':'Faça uma auditoria aqui e descubra como deve ser a estrutura ideal para o seu negócio.',
 'Equipe & Organização Ideal':'Faça uma auditoria aqui e descubra como deve ser a organização certa da sua equipe.',
 'Processos & Produtividade':'Faça uma auditoria aqui e descubra como devem ser feitos corretamente os processos no seu negócio.',
 'Tecnologia, IA & Automação':'Faça uma auditoria aqui e descubra onde tecnologia, IA e automação podem economizar tempo e reduzir falhas.',
 'Modelo de Gestão':'Faça uma auditoria aqui e descubra como sua rotina de gestão pode ficar mais clara, simples e eficiente.',
 'Capacidade & Gargalos':'Faça uma auditoria aqui e descubra o que está travando crescimento, entrega ou atendimento.',
 'Identidade & Apresentação da Marca':'Faça uma auditoria aqui e descubra se sua marca transmite profissionalismo e confiança.',
 'Posicionamento & Diferenciação':'Faça uma auditoria aqui e descubra como sua empresa pode ser lembrada pelo motivo certo.',
 'Autoridade, Confiança & Reputação':'Faça uma auditoria aqui e descubra como aumentar confiança antes da pessoa comprar.',
 'Mercado, Oportunidades & Validação':'Faça uma auditoria aqui e descubra oportunidades reais antes de investir energia no caminho errado.',
 'Público & Comportamento':'Faça uma auditoria aqui e descubra quem é seu público e como ele decide comprar.',
 'Concorrência & Benchmarking':'Faça uma auditoria aqui e descubra o que observar nos concorrentes sem copiar literalmente.',
 'O que Posso Vender?':'Faça uma auditoria aqui e descubra quais ofertas fazem mais sentido para o seu momento.',
 'Portfólio Estratégico & Performance':'Faça uma auditoria aqui e descubra quais produtos ou serviços merecem prioridade.',
 'Validação Individual de Produto/Serviço':'Faça uma auditoria aqui e descubra se uma oferta específica tem clareza, demanda e potencial.',
 'Preço, Valor & Oferta':'Faça uma auditoria aqui e descubra se seu preço e sua oferta comunicam valor suficiente.',
 'Presença Física Ideal':'Faça uma auditoria aqui e descubra como seu espaço físico pode passar mais confiança.',
 'Presença Digital Ideal':'Faça uma auditoria aqui e descubra onde sua empresa precisa aparecer melhor no digital.',
 'Integração Físico + Digital / Omnichannel':'Faça uma auditoria aqui e descubra se seus canais físicos e digitais trabalham juntos.',
 'Estratégia de Divulgação':'Faça uma auditoria aqui e descubra como divulgar com mais foco e menos tentativa solta.',
 'Conteúdo & Comunicação':'Faça uma auditoria aqui e descubra que tipo de conteúdo sua empresa deveria publicar.',
 'Orgânico & Descoberta':'Faça uma auditoria aqui e descubra como ser encontrado sem depender só de anúncios.',
 'Tráfego Pago':'Faça uma auditoria aqui e descubra se anúncios fazem sentido e o que precisa estar pronto antes.',
 'Campanhas & Criativos':'Faça uma auditoria aqui e descubra como criar campanhas com tema, oferta e mensagem mais fortes.',
 'Mensuração & Otimização':'Faça uma auditoria aqui e descubra quais números acompanhar para melhorar resultado.',
 'Funil & Processo Comercial':'Faça uma auditoria aqui e descubra como transformar interesse em venda com mais organização.',
 'Atendimento, Argumentação & Conversão':'Faça uma auditoria aqui e descubra como melhorar atendimento, argumentos e fechamento.',
 'CRM & Gestão Comercial':'Faça uma auditoria aqui e descubra como organizar contatos, retornos e oportunidades.',
 'Equipe & Performance Comercial':'Faça uma auditoria aqui e descubra como sua equipe pode vender com mais padrão e clareza.',
 'Jornada do Cliente':'Faça uma auditoria aqui e descubra como melhorar cada etapa da experiência do cliente.',
 'Experiência & Branding na Prática':'Faça uma auditoria aqui e descubra se a experiência entregue combina com a promessa da marca.',
 'Satisfação & Pós-venda':'Faça uma auditoria aqui e descubra como acompanhar clientes depois da compra.',
 'Fidelização, Retenção & Recompra':'Faça uma auditoria aqui e descubra como fazer clientes voltarem mais vezes.',
 'Indicação & Advocacia da Marca':'Faça uma auditoria aqui e descubra como transformar clientes satisfeitos em indicação.',
 'Gestão Financeira & Rentabilidade':'Faça uma auditoria aqui e descubra se o negócio está crescendo com saúde financeira.',
 'Indicadores, Metas & Planejamento':'Faça uma auditoria aqui e descubra quais metas e indicadores devem guiar suas decisões.',
 'Gestão de Marketing':'Faça uma auditoria aqui e descubra como organizar marketing com rotina, prioridade e resultado.',
 'Gestão de Redes & Conteúdo':'Faça uma auditoria aqui e descubra como manter redes sociais com consistência e propósito.',
 'Gestão Comercial':'Faça uma auditoria aqui e descubra como acompanhar vendas, atendimento e follow-up.',
 'Gestão de Pessoas':'Faça uma auditoria aqui e descubra como organizar melhor funções, responsabilidades e evolução da equipe.',
 'Riscos & Capacidade':'Faça uma auditoria aqui e descubra riscos operacionais antes que eles prejudiquem crescimento.',
 'Plano de Evolução Contínua':'Faça uma auditoria aqui e descubra o próximo plano de melhoria para evoluir sem se perder.'
};
function analysisSubDescription(name){return analysisSubDescriptions[name]||`Faça uma auditoria aqui e descubra como melhorar ${String(name||'este ponto').toLowerCase()} no seu negócio.`}
function renderAnalyses(){const analysisImgs={...analysisImageMap};document.getElementById('analysisGrid').innerHTML=analyses.map((a,i)=>{const saved=state.analysisFav.includes(a[0]);return `<article class="analysisCard"><div class="analysisPhoto analysisPhotoFallback"><img src="${analysisImgs[a[0]]||imgs.team}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove()"><span>ÁREA ${i+1}</span><button class="analysisFav ${saved?'saved':''}" data-analysis-fav="${a[0]}">${saved?'♥':'♡'}</button></div><div class="analysisContent"><h3>${a[2]}</h3><h4>${a[3]}?</h4><p>O MARK investiga esta área considerando seu momento, objetivo, capacidade e contexto — sem procurar uma resposta padrão.</p><div class="subs">${a[4].map(s=>`<button data-sub="${a[0]}|${s}"><span><b>${s}</b><small>${analysisSubDescription(s)}</small></span><strong>Analisar</strong></button>`).join('')}</div><div class="analysisActions"><button class="analysisFull" data-analysis="${a[0]}">Analisar área completa</button><button class="seeSubs">Ver ${a[4].length} subanálises</button></div></div></article>`}).join('');bindAnalysisFav();document.querySelectorAll('[data-analysis]').forEach(b=>b.onclick=()=>startAnalysis(b.dataset.analysis));document.querySelectorAll('[data-sub]').forEach(b=>b.onclick=()=>startAnalysis(...b.dataset.sub.split('|')))}
...
const analysisDepthPoints={
 'Estrutura Ideal do Negócio':['Clareza de modelo de negócio','Prioridades por área','Oferta principal definida','Canais mais importantes','Rotina de acompanhamento','Pontos que dependem de dono, equipe ou processo'],
 'Equipe & Organização Ideal':['Funções e responsabilidades','Comunicação interna','Treinamento recorrente','Capacidade atual','Gargalos de execução','Indicadores por equipe'],
 'Processos & Produtividade':['Etapas repetitivas','Retrabalho','Prazos e responsáveis','Ferramentas usadas','Automação possível','Ponto onde o cliente espera mais'],
 'Tecnologia, IA & Automação':['Sistemas atuais','Dados que podem ser centralizados','Tarefas que podem virar automação','Uso de IA com contexto','Riscos de depender de processo manual','Próxima automação de maior impacto'],
 'Identidade & Apresentação da Marca':['Percepção desejada','Consistência visual','Tom de voz','Fotografia e vídeo','Materiais comerciais','Coerência físico/digital'],
 'Posicionamento & Diferenciação':['Promessa central','Diferencial real','Provas disponíveis','Território não ocupado','Concorrentes que dominam percepção','Mensagem que precisa ser testada'],
 'Autoridade, Confiança & Reputação':['Provas legítimas','Avaliações','Cases ou portfólio','Presença de especialistas','Resposta a dúvidas','Reputação em canais de busca'],
 'Mercado, Oportunidades & Validação':['Demanda percebida','Tendências do nicho','Oportunidades locais','Barreiras de compra','Validação de oferta','Riscos antes de investir'],
 'Público & Comportamento':['Perfil de cliente ideal','Nível de consciência','Motivos de compra','Objeções','Canais usados','Momento de decisão'],
 'Concorrência & Benchmarking':['Concorrentes diretos','Alternativas indiretas','O que cada concorrente domina','O que aprender','O que não copiar','Território livre'],
 'O que Posso Vender?':['Produto principal','Serviço complementar','Pacotes','Recorrência','Oferta de entrada','Oferta premium'],
 'Portfólio Estratégico & Performance':['Itens que vendem mais','Itens com melhor margem','Itens que atraem cliente','Itens que geram recompra','Itens que confundem o cliente','Ordem de exposição'],
 'Validação Individual de Produto/Serviço':['Problema resolvido','Público específico','Preço aceitável','Prova necessária','Canal ideal','Critério de manter, ajustar ou remover'],
 'Preço, Valor & Oferta':['Custo e margem','Valor percebido','Comparação com mercado','Condições de pagamento','Desconto sustentável','Argumentos de valor'],
 'Presença Física Ideal':['Fachada','Vitrine','Circulação','Sensorial','Atendimento','Integração com digital'],
 'Presença Digital Ideal':['Site','Instagram','WhatsApp','Google','Provas','Caminho de conversão'],
 'Integração Físico + Digital / Omnichannel':['Loja para WhatsApp','WhatsApp para compra','QR Codes','Retirada e entrega','Histórico do cliente','Mensuração de origem'],
 'Estratégia de Divulgação':['Objetivo da campanha','Público','Canal','Mensagem','Orçamento','Métrica'],
 'Conteúdo & Comunicação':['Educação','Autoridade','Prova','Relacionamento','Venda','Bastidores'],
 'Orgânico & Descoberta':['SEO','Google','Redes sociais','Indicação','Comunidades','Conteúdo evergreen'],
 'Tráfego Pago':['Oferta pronta','Página de destino','Público','Criativo','Verba','Métrica de conversão'],
 'Campanhas & Criativos':['Tema','Canal','Criativo','CTA','Antecedência','Resultado esperado'],
...
function renderAnalysisDetail(id,sub=''){
 ...
 document.getElementById('analysisGenerate')?.addEventListener('click',async()=>{
   ...
   const canUseAnalysisAi=hasProAccess();
   box.innerHTML=canUseAnalysisAi
    ?`<div class="analysisResultCard"><span class="eyebrow">RESULTADO DA ANÁLISE</span><h3>${score}% de adequação inicial</h3><p>Interpretando respostas, checklist, Perfil da Empresa e metodologia cadastrada no MARK.IA...</p><div class="analysisAiLoading">● ● ● Gerando interpretação inteligente</div></div>`
    :`<div class="analysisResultCard"><span class="eyebrow">RESULTADO INICIAL GRATUITO</span><h3>${score}% de adequação inicial</h3><p>Seu resultado básico foi calculado pelo checklist. A interpretação completa e contextual do MARK.IA é liberada nos planos PRO e Premium.</p></div>`;
   let ai=null;
   try{
     if(canUseAnalysisAi&&mivUser&&mivSupabase){
   ...
   box.innerHTML=`<div class="analysisResultCard"><span class="eyebrow">${canUseAnalysisAi?'RESULTADO DA ANÁLISE':'RESULTADO INICIAL GRATUITO'}</span><h3>${score}% de adequação inicial</h3><p>${ai?.score_interpretation||(canUseAnalysisAi?'Este resultado combina suas respostas específicas com o Perfil da Empresa.':'Este resultado básico foi calculado a partir do checklist preenchido.')}</p>${ai?.summary?`<section class="analysisAiSummary"><span class="eyebrow">LEITURA DO CENÁRIO</span><p>${ai.summary}</p></section>`:''}${list('O QUE ANALISAMOS',ai?.what_we_analyzed)}${list('O QUE ENCONTRAMOS',ai?.what_we_found)}${list('FATOS INFORMADOS',ai?.facts)}${list('SINAIS OBSERVADOS',ai?.signals)}${list('HIPÓTESES',ai?.hypotheses)}${list('O QUE ISSO REVELA',ai?.what_it_reveals)}${list('PONTOS FORTES',ai?.strengths)}${list('GARGALOS IDENTIFICADOS',ai?.bottlenecks)}${priorities.length?`<section class="analysisAiSection"><span class="eyebrow">PRIORIDADES</span><div class="analysisPriorities">${priorities.map((x,i)=>`<div><strong>${i+1}</strong><span>${x}</span></div>`).join('')}</div></section>`:'<p>Nenhum ponto prioritário foi identificado nesta etapa.</p>'}${list('DECISÃO RECOMENDADA',ai?.recommended_decision)}${list('PRÓXIMOS PASSOS',ai?.next_steps)}${list('NÃO FAÇA AGORA',ai?.do_not_do_now)}${list('PLANO DE AÇÃO',ai?.action_plan)}${list('REVISAR EM',ai?.review_in)}${list('PONTOS DE ATENÇÃO',ai?.watchouts)}${canUseAnalysisAi?(!ai?analysisSpecialistView(score,priorities):'<p class="specialistHuman">Interpretação gerada pelo MARK.IA com a metodologia cadastrada e os dados fornecidos. Não representa revisão humana individual.</p>'):`<div class="specialistView"><span class="eyebrow">ANÁLISE COMPLETA</span><h3>Desbloqueie a interpretação contextual</h3><p>PRO e Premium usam Perfil da Empresa, respostas e metodologia do MARK.IA para gerar leitura completa e prioridades personalizadas.</p><button class="primary" data-analysis-upgrade>Ver planos</button></div>`}<button class="outline" onclick="window.print()">Imprimir relatório</button></div>`;
   document.querySelector('[data-analysis-upgrade]')?.addEventListener('click',()=>openPlanPaywall('Análises completas com MARK.IA'));
...
function startAnalysis(id,sub=''){if(id==='completa'&&!hasProAccess()){openPlanPaywall('Análise Empresarial Completa');return}openAnalysisDetail(id,sub)}

const testimonialExamples=[
 ['Clínica de estética','Marketing','O calendário deixou minhas campanhas menos improvisadas e me ajudou a planejar datas com objetivo, público e canal.','Calendário inteligente'],
 ['Loja de roupas','Vendas','O card de ticket médio me fez enxergar combos, segunda peça e benefício progressivo sem depender só de desconto.','Ticket médio'],
 ['Consultório odontológico','Marca','O branding digital mostrou que meu site, Instagram e WhatsApp não pareciam a mesma clínica. Ficou muito claro o que ajustar.','Branding digital'],
 ['Restaurante delivery','Marketing','A parte de Google e WhatsApp ajudou a organizar fotos, avaliações, cardápio e atendimento para transformar busca em pedido.','Canais'],
 ['Academia','Gestão','As análises deram uma visão mais prática dos gargalos de equipe, recorrência e retenção de alunos.','Análises'],
 ['Advocacia empresarial','Marca','O checklist de autoridade e reputação digital ajudou a deixar a comunicação mais sóbria e confiável.','Autoridade'],
 ['Pet shop','Vendas','O card de fidelização abriu ideias simples de recompra, lembrete e indicação para clientes recorrentes.','Fidelização'],
 ['Padaria','Marketing','O marketing local conectou vitrine, fluxo de pessoas, Google e campanhas do bairro em uma lógica só.','Marketing local'],
 ['Escola de idiomas','Gestão','A Central ficou útil para salvar ideias, relatórios e pontos que eu queria revisar depois com calma.','Minha Central'],
 ['Imobiliária','Vendas','O follow-up ficou muito mais organizado por estágio, responsável e próxima ação.','Follow-up'],
 ['Salão de beleza','Marketing','O Instagram saiu de “postar qualquer coisa” para educação, prova, relacionamento e venda.','Instagram'],
 ['Software house','Gestão','A análise de tecnologia, processos e produtividade ajudou a separar problema técnico de problema de gestão.','Análise empresarial'],
 ['Loja de móveis','Marca','A identidade aplicada ao marketing mostrou onde a marca precisava aparecer melhor: vitrine, preço, fotos e proposta.','Identidade visual'],
 ['Hamburgueria','Vendas','O montador de oferta ajudou a pensar combos, adicionais, entrega e margem antes da campanha.','Oferta'],
 ['Clínica médica','Marketing','O sistema deixou mais evidente quais informações precisavam estar no Google e no site para gerar confiança.','Google e site'],
 ['Contabilidade','Marca','A diferenciação competitiva ajudou a parar de falar só “atendimento personalizado” e pensar em prova e processo.','Diferenciação'],
 ['Barbearia','Marketing','A comunicação física ganhou importância: fachada, vitrine, aroma, música e experiência começaram a fazer parte do plano.','Branding físico'],
 ['Loja de cosméticos','Vendas','Os brindes passaram a ter objetivo: lançamento, recompra, VIP e indicação, sem virar custo solto.','Brindes'],
 ['Corretor de seguros','Vendas','O WhatsApp ficou mais consultivo, com perguntas, objeções e retorno planejado.','Script de WhatsApp'],
 ['Cafeteria','Marketing','Gostei de enxergar datas, lançamentos e ações locais junto com posts e campanhas.','Calendário'],
 ['Fisioterapeuta','Marca','O posicionamento ficou mais claro para não parecer genérico em um mercado cheio de opções parecidas.','Posicionamento'],
 ['Boutique','Marketing','A parte de conteúdo me ajudou a separar vitrine, prova social, bastidores e venda.','Conteúdo'],
 ['Construtora','Marca','O card de prova e reputação mostrou que portfólio, fotos e apresentação comercial precisavam conversar melhor.','Marca'],
 ['Marmitaria','Vendas','Compreendi melhor como organizar recompra semanal, assinatura e relacionamento com cliente recorrente.','Recorrência'],
 ['Designer freelancer','Gestão','O sistema ajudou a transformar diagnóstico em prioridade e não em uma lista infinita de coisas soltas.','Prioridades'],
 ['Ótica','Marketing','O marketing local destacou Google, avaliações, vitrine, fluxo e parcerias do entorno.','Marketing local'],
 ['Escola infantil','Marca','A comunicação ficou mais coerente entre confiança, acolhimento, prova e rotina.','Branding digital'],
 ['Oficina mecânica','Vendas','O follow-up de orçamento virou processo, com motivo de perda e valor potencial.','Follow-up'],
 ['Nutricionista','Marketing','As ideias de conteúdo ficaram mais conectadas com dúvidas reais dos pacientes.','Conteúdo e autoridade'],
 ['Arquiteta','Marca','A identidade visual aplicada mostrou como proposta, portfólio, Instagram e apresentação precisam ter o mesmo nível.','Identidade'],
 ['Loja de presentes','Vendas','O card de oferta ajudou a montar kits e benefícios por ocasião sem bagunçar preço.','Oferta'],
 ['Psicóloga','Marca','A parte de reputação e tom de voz ajudou a equilibrar autoridade com cuidado.','Tom de voz'],
 ['Pizzaria','Marketing','A integração loja, WhatsApp e Instagram ficou muito mais fácil de visualizar.','Canais'],
 ['Curso livre','Gestão','O Aprenda & Aplique virou uma área boa para estudar e transformar em ação com o MARK.','Aprenda & Aplique'],
 ['Energia solar','Vendas','O script de WhatsApp ajudou a diagnosticar necessidade antes de apresentar proposta.','WhatsApp'],
 ['Supermercado','Marketing','Datas, campanhas, lançamentos e ações de relacionamento ficaram mais fáceis de organizar.','Calendário'],
 ['Estúdio de pilates','Clientes','O pós-venda e a reativação ficaram mais claros para alunos inativos.','Clientes'],
 ['Fotógrafa','Marketing','A direção fotográfica e o estilo visual entraram como parte real da estratégia.','Identidade aplicada'],
 ['Auto elétrica','Marca','O Google Perfil da Empresa apareceu como prioridade para descoberta local e confiança.','Google'],
 ['Doceria','Vendas','O ticket médio ajudou a pensar kits, segunda compra, encomendas e datas especiais.','Ticket'],
 ['Mentor de carreira','Marca','A diferenciação ficou menos abstrata e mais ligada a método, prova e entrega.','Diferenciação'],
 ['Loja de calçados','Marketing','A vitrine, os best-sellers e o WhatsApp passaram a trabalhar juntos.','Marketing físico'],
 ['Produtora de vídeo','Gestão','As análises ajudaram a enxergar capacidade, gargalos e prioridades de crescimento.','Análises'],
 ['Jardim e paisagismo','Vendas','A oferta ficou mais clara com prazo, entrega, escopo e próximos passos.','Oferta'],
 ['Clínica veterinária','Clientes','O ciclo de vida do cliente ajudou a pensar lembretes, retornos e cuidados contínuos.','Fidelização'],
 ['Papelaria','Marketing','O calendário de datas e campanhas locais deu ideias práticas para movimento na loja.','Calendário'],
 ['Consultoria B2B','Vendas','O funil de conversa, proposta e follow-up ficou mais objetivo.','Vendas'],
...
function updateCompanyProfileToggle(){
 const panel=document.getElementById('companyProfilePanel'),btn=document.getElementById('companyProfileToggle');
 if(!panel||!btn)return;
 const open=!!panel.open;
 btn.textContent=open?'Ocultar informações':'Abrir informações';
 btn.setAttribute('aria-expanded',String(open));
}

function reportMetricRows(meta={}){
 const rows=[];
 if(meta.score!=null)rows.push(['Pontuação',`${meta.score}%`]);
 if(meta.progress!=null)rows.push(['Progresso',`${meta.progress}%`]);
 if(meta.correct!=null&&meta.total!=null)rows.push(['Pontos adequados',`${meta.correct} de ${meta.total}`]);
...
function backToReportAnalysis(){
 const id=state.currentReport?.meta?.sourceItemId||state.current?.id;
 if(id&&getItem(id)){openItem(id);return}
 route('central');
}
...
function renderFullSavedReport(report){
 ...
 body.querySelectorAll('[data-report-action]').forEach(btn=>btn.onclick=()=>({print:printCurrentReport,pdf:saveCurrentReportPdf,whatsapp:shareCurrentReportWhatsApp,'back-analysis':backToReportAnalysis}[btn.dataset.reportAction]||(()=>{}))());
}
...
function fillCompanyProfileForm(){
 ...
 Object.entries(companyProfileFields).forEach(([id,key])=>{if(id==='cpNiche'||id==='cpSubniche')return;const el=document.getElementById(id);if(el)el.value=p[key]||''});
 renderCompanyNicheSelects(p);
...
async function saveCompanyProfileForm(){
 ...
 const p=getCompanyProfile();
 Object.entries(companyProfileFields).forEach(([id,key])=>{if(id==='cpNiche'||id==='cpSubniche')return;const el=document.getElementById(id);if(el)p[key]=el.value.trim()});
 p.niche=selectedWithOther('cpNiche');
 p.subniche=selectedWithOther('cpSubniche');
...
document.getElementById('companyProfilePanel')?.addEventListener('toggle',updateCompanyProfileToggle);
document.getElementById('companyProfileToggle')?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const panel=document.getElementById('companyProfilePanel');if(panel){panel.open=!panel.open;updateCompanyProfileToggle();}});
bindNicheSelectWithOther('cpNiche',e=>{const raw=e.target.value===OTHER_NICHE_VALUE?selectedWithOther('cpNiche'):e.target.value;renderCompanyNicheSelects({niche:raw,subniche:''});const sub=document.getElementById('cpSubniche');if(sub)sub.value=''});
bindNicheSelectWithOther('cpSubniche');
...
function initEcosystem(){
  try{hydrateMarketCache()}catch(e){console.warn('Market cache init',e)}
  try{hydrateCompetitorNames()}catch(e){console.warn('Competitor names init',e)}
  try{renderNiches()}catch(e){console.warn('Niches init',e)}
  try{renderRegisterNicheSelect();renderCalendarNicheSelect(state.profile.niche)}catch(e){console.warn('Niche selects init',e)}
  try{renderAnalyses()}catch(e){console.warn('Analyses init',e)}
  try{renderLogos()}catch(e){console.warn('Logos init',e)}
  try{renderTestimonials()}catch(e){console.warn('Testimonials init',e)}
...
document.getElementById('applyNiche').onclick=()=>{const v=document.getElementById('nicheSearch').value.trim();if(!v)return toast('Digite ou escolha um nicho.');const hit=Object.keys(niches).find(n=>n.toLowerCase()===v.toLowerCase());applyTheme(hit||v,hit?niches[hit].theme:'clean',true)};document.getElementById('focusNiche').onclick=()=>document.getElementById('nicho').scrollIntoView({behavior:'smooth'});document.getElementById('editProfile').onclick=()=>route('home');document.getElementById('favBtn').onclick=()=>state.current&&toggleFav(state.current.id);document.getElementById('useBtn').onclick=()=>toast('Use o botão desta página para iniciar a ferramenta.');
document.querySelectorAll('[data-route]').forEach(b=>b.onclick=()=>{document.body.classList.remove('menu-open');document.getElementById('mobileMenu')?.setAttribute('aria-expanded','false');route(b.dataset.route)});document.querySelectorAll('[data-scroll]').forEach(b=>b.onclick=()=>{document.body.classList.remove('menu-open');document.getElementById('mobileMenu')?.setAttribute('aria-expanded','false');route('home');setTimeout(()=>document.getElementById(b.dataset.scroll)?.scrollIntoView({behavior:'smooth'}),60)});document.getElementById('mobileMenu')?.addEventListener('click',()=>{const open=document.body.classList.toggle('menu-open');document.getElementById('mobileMenu')?.setAttribute('aria-expanded',open?'true':'false')});document.querySelectorAll('[data-prev]').forEach(b=>b.onclick=()=>document.getElementById(b.dataset.prev).scrollBy({left:-580,behavior:'smooth'}));document.querySelectorAll('[data-next]').forEach(b=>b.onclick=()=>document.getElementById(b.dataset.next).scrollBy({left:580,behavior:'smooth'}));document.querySelectorAll('[data-tool-filter]').forEach(b=>b.onclick=()=>{state.toolFilter=b.dataset.toolFilter;document.querySelectorAll('[data-tool-filter]').forEach(x=>x.classList.toggle('active',x===b));renderTools()});document.querySelectorAll('[data-learn-filter]').forEach(b=>b.onclick=()=>{state.learnFilter=b.dataset.learnFilter;document.querySelectorAll('[data-learn-filter]').forEach(x=>x.classList.toggle('active',x===b));renderLearn()});
document.getElementById('refreshTrends')?.addEventListener('click',refreshMarketTrends);
document.getElementById('refreshCompetitors')?.addEventListener('click',()=>refreshCompetitors(false));
document.getElementById('suggestCompetitors')?.addEventListener('click',()=>refreshCompetitors(true));
document.getElementById('competitorNames')?.addEventListener('input',()=>saveCompetitorNames());
document.querySelectorAll('[data-testimonial-filter]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-testimonial-filter]').forEach(x=>x.classList.toggle('active',x===b));renderTestimonials(b.dataset.testimonialFilter)});
document.getElementById('closeVideoModal')?.addEventListener('click',closeLearningVideo);document.getElementById('videoOverlay')?.addEventListener('click',closeLearningVideo);
...
async function sendMarkQuestion(q){
 ...
 if(!mivUser||!mivSupabase){openAuth('login');toast('Entre na sua conta para conversar com o MARK.IA.');return}
 m.insertAdjacentHTML('beforeend',`<p class="user">${markEsc(q)}</p>`);m.insertAdjacentHTML('beforeend','<p class="markThinking" id="markThinking">MARK está analisando seu contexto<span>...</span></p>');m.scrollTop=m.scrollHeight;
 try{
  const {data:sessionData,error}=await mivSupabase.auth.getSession();if(error)throw error;const token=sessionData?.session?.access_token;if(!token)throw new Error('Sessão expirada. Entre novamente.');
  const current=state.current||null;const context={route:state.route||'home',item_id:current?.id||'',title:current?.title||document.getElementById('ctxTitle')?.textContent||''};
  const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({question:q,history:markConversation.slice(-8),context})});
  const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível conversar com o MARK.IA.');
  document.getElementById('markThinking')?.remove();m.insertAdjacentHTML('beforeend',markAnswerHtml(data.answer,data.sources));
  markConversation.push({role:'user',text:q},{role:'assistant',text:data.answer});markConversation=markConversation.slice(-10);m.scrollTop=m.scrollHeight;
 }catch(err){document.getElementById('markThinking')?.remove();m.insertAdjacentHTML('beforeend',`<p><b>Não consegui responder agora.</b><br>${markEsc(err.message)}</p>`);m.scrollTop=m.scrollHeight}
}
...
let calDynamicEvents=[];
let calDynamicSources=[];
function calEsc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
...
async function openCalendar(){const modal=document.getElementById('calendarModal');modal.classList.add('open');calDate=new Date();fillCalSelectors();const local=readCalendarPrefs();renderCalendarNicheSelect(local.niche||state.profile.niche||'');document.getElementById('calCity').value=local.city||state.profile.city||'';document.getElementById('calCustomDates').value=local.customText||'';calDynamicEvents=Array.isArray(local.dynamicEvents)?local.dynamicEvents:[];calDynamicSources=Array.isArray(local.dynamicSources)?local.dynamicSources:[];if(mivUser&&mivSupabase){try{const {data}=await mivSupabase.from('marketing_calendar_profiles').select('*').eq('user_id',mivUser.id).maybeSingle();if(data){renderCalendarNicheSelect(data.niche||selectedWithOther('calNiche'));document.getElementById('calCity').value=data.city||document.getElementById('calCity').value;document.getElementById('calCustomDates').value=data.custom_text||'';calDynamicEvents=Array.isArray(data.dynamic_events)?data.dynamic_events:[];calDynamicSources=Array.isArray(data.dynamic_sources)?data.dynamic_sources:[];saveCalendarPrefs({niche:data.niche||'',city:data.city||'',customText:data.custom_text||'',customEvents:Array.isArray(data.custom_events)?data.custom_events:[],dynamicEvents:calDynamicEvents,dynamicSources:calDynamicSources,savedAt:data.updated_at,dynamicUpdatedAt:data.dynamic_updated_at})}}catch(e){console.warn('[Calendar cloud load]',e)}}renderCalendar();updateCalSavedStatus();renderCalSources()}
...
function dynamicEventsFor(year,month,day){return calDynamicEvents.filter(e=>{const d=String(e.date||'').split('-');return +d[0]===year&&+d[1]===month&&+d[2]===day}).map(e=>({name:e.name,type:e.type||'sector',relevance:e.relevance||'medium',reason:e.reason||''}))}
function allEventsFor(month,day){const fixed=calEvents[`${month}-${day}`]||[],custom=customCalendarEvents()[`${month}-${day}`]||[],dyn=dynamicEventsFor(calDate.getFullYear(),month,day);return [...fixed.map(x=>({name:x,type:'base'})),...dyn,...custom.map(x=>({name:x,type:'custom'}))]}
...
function updateCalSavedStatus(){const p=readCalendarPrefs(),el=document.getElementById('calSavedStatus');if(!el)return;const n=(p.customEvents||[]).length,dyn=calDynamicEvents.filter(e=>String(e.date||'').startsWith(`${calDate.getFullYear()}-${String(calDate.getMonth()+1).padStart(2,'0')}`)).length;el.textContent=p.savedAt?`Configuração salva · ${n} data(s) própria(s) · ${dyn} data(s) pesquisada(s) neste mês.`:'Configure nicho/cidade e salve para personalizar o calendário.'}
async function saveImportantCalendarDates(){const niche=selectedWithOther('calNiche'),city=document.getElementById('calCity').value.trim(),customText=document.getElementById('calCustomDates').value.trim(),customEvents=parseCustomDates(customText),savedAt=new Date().toISOString(),payload={niche,city,customText,customEvents,dynamicEvents:calDynamicEvents,dynamicSources:calDynamicSources,savedAt};saveCalendarPrefs(payload);if(mivUser&&mivSupabase){const {error}=await mivSupabase.from('marketing_calendar_profiles').upsert({user_id:mivUser.id,niche,city,custom_text:customText,custom_events:customEvents,dynamic_events:calDynamicEvents,dynamic_sources:calDynamicSources,updated_at:savedAt},{onConflict:'user_id'});if(error)console.warn('[Calendar cloud save]',error)}updateCalSavedStatus();renderCalendar();toast(customEvents.length?`${customEvents.length} data(s) importante(s) salva(s).`:'Preferências do calendário salvas.')}
async function refreshCalendarResearch(){if(!mivUser){toast('Entre na sua conta para pesquisar datas do nicho e da cidade.');return}const btn=document.getElementById('calRefreshResearch');btn.disabled=true;btn.textContent='Pesquisando...';try{const niche=selectedWithOther('calNiche'),city=document.getElementById('calCity').value.trim();const {data:{session}}=await mivSupabase.auth.getSession();const r=await fetch('/api/calendar-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session?.access_token||''}`},body:JSON.stringify({action:'discover_events',year:calDate.getFullYear(),month:calDate.getMonth()+1,niche,city})});const data=await r.json();if(!r.ok)throw new Error(data.error||'Falha na pesquisa');const prefix=`${calDate.getFullYear()}-${String(calDate.getMonth()+1).padStart(2,'0')}`;calDynamicEvents=[...calDynamicEvents.filter(e=>!String(e.date||'').startsWith(prefix)),...(data.events||[])];calDynamicSources=data.sources||[];const p=readCalendarPrefs();saveCalendarPrefs({...p,niche,city,dynamicEvents:calDynamicEvents,dynamicSources:calDynamicSources,dynamicUpdatedAt:new Date().toISOString(),savedAt:new Date().toISOString()});if(mivUser&&mivSupabase)await mivSupabase.from('marketing_calendar_profiles').upsert({user_id:mivUser.id,niche,city,custom_text:document.getElementById('calCustomDates').value.trim(),custom_events:parseCustomDates(document.getElementById('calCustomDates').value),dynamic_events:calDynamicEvents,dynamic_sources:calDynamicSources,dynamic_updated_at:new Date().toISOString(),updated_at:new Date().toISOString()},{onConflict:'user_id'});renderCalendar();renderCalSources();updateCalSavedStatus();toast(`${(data.events||[]).length} data(s) contextual(is) encontrada(s) para o mês.`)}catch(e){console.error(e);toast(e.message||'Não foi possível pesquisar eventos agora.')}finally{btn.disabled=false;btn.textContent='↻ Atualizar datas do nicho/cidade'}}
async function generateCalendarIdeas(event,d,btn){if(!mivUser){toast('Entre na sua conta para gerar campanhas.');return}btn.disabled=true;btn.textContent='MARK.IA criando...';const target=document.getElementById('calGeneratedIdea');target.innerHTML='<div class="calAiLoading">Analisando empresa, data e oportunidade...</div>';try{const {data:{session}}=await mivSupabase.auth.getSession();const r=await fetch('/api/calendar-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session?.access_token||''}`},body:JSON.stringify({action:'generate_ideas',event:event.name,date:calEventDate(d),reason:event.reason||suggestAngle(event.name,relevanceFor(event.name,event),event),niche:selectedWithOther('calNiche'),city:document.getElementById('calCity').value.trim()})});const data=await r.json();if(r.status===403){openPlanPaywall('Ideias completas do Calendário');return}if(!r.ok)throw new Error(data.error||'Falha ao gerar campanha');const x=data.idea||{},list=v=>Array.isArray(v)?v:[];target.innerHTML=`<div class="calAiResult"><span class="eyebrow">MARK.IA · CAMPANHA PERSONALIZADA</span><h3>${calEsc(x.campaign?.name||event.name)}</h3><p><b>Estratégia:</b> ${calEsc(x.strategy||'')}</p><div class="calAiCols"><div><h4>Ideias de criativos</h4><ul>${list(x.creative_ideas).map(i=>`<li>${calEsc(i)}</li>`).join('')}</ul></div><div><h4>Campanha</h4><p><b>Objetivo:</b> ${calEsc(x.campaign?.objective||'')}</p><p><b>Mecânica:</b> ${calEsc(x.campaign?.mechanic||'')}</p></div></div><h4>Mensagem sugerida</h4><p class="calCopy">${calEsc(x.copy||'')}</p><p><b>CTA:</b> ${calEsc(x.cta||'')}</p><p><b>Canais:</b> ${list(x.channels).map(calEsc).join(' · ')}</p><p><b>Timing:</b> ${list(x.timing).map(calEsc).join(' · ')}</p>${list(x.avoid).length?`<p><b>Evite:</b> ${list(x.avoid).map(calEsc).join(' · ')}</p>`:''}<button class="secondary" id="calSaveIdea">Salvar ideia na Minha Central</button></div>`;document.getElementById('calSaveIdea')?.addEventListener('click',()=>saveCalendarIdea(event,d,x))}catch(e){console.error(e);target.innerHTML=`<div class="calAiError">${calEsc(e.message||'Não foi possível gerar a campanha agora.')}</div>`}finally{btn.disabled=false;btn.textContent='✨ Gerar campanha com MARK.IA'}}
...
const MIV_PRODUCTION_ORIGIN='https://miv-ecosystem.vercel.app';
...
function renderSubscriptionManager(){
 ...
 if(down){down.onclick=()=>scheduleDowngrade();down.style.cssText='background:none;border:0;box-shadow:none;padding:4px 0;color:#75807d;text-decoration:underline;text-underline-offset:3px;font:inherit;font-size:13px;cursor:pointer'}
...
function runPendingAuthAction(){
 ...
 if(action.type==='refreshCompetitors'){route('home');setTimeout(()=>{document.getElementById('concorrentes')?.scrollIntoView({behavior:'smooth',block:'start'});refreshCompetitors(!!action.suggest,action.channel||'',action.link||'')},180);return true}
 if(action.type==='paywall'&&action.itemId){route('home');setTimeout(()=>{const item=getItem(action.itemId),escaped=window.CSS&&CSS.escape?CSS.escape(action.itemId):String(action.itemId).replace(/"/g,'\\"');if(item){document.querySelector(`[data-open="${escaped}"]`)?.scrollIntoView({behavior:'smooth',block:'center'});openPaywall(item)}},180);return true}
 if(action.type==='openItem'&&action.itemId){route('home');setTimeout(()=>openItem(action.itemId),180);return true}
 return false;
}
...
let learningRows=[],learningTracksRows=[],centralExtras={ideas:[],purchases:[],progress:[]};
function youtubeIdFromUrl(url){const s=String(url||'');const m=s.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([A-Za-z0-9_-]{6,})/);return m?m[1]:''}
function openLearningVideo(item){
 const modal=document.getElementById('videoModal'),overlay=document.getElementById('videoOverlay'),wrap=document.getElementById('videoFrameWrap');if(!modal||!overlay||!wrap)return;
 document.getElementById('videoTitle').textContent=item.title||'Aula MivCast';
 document.getElementById('videoDesc').textContent=item.desc||'';
 document.getElementById('videoLabel').textContent=`APRENDA & APLIQUE · ${String(item.format||'VÍDEO').toUpperCase()}`;
 wrap.innerHTML=item.learningVideoId?`<iframe src="https://www.youtube.com/embed/${item.learningVideoId}?autoplay=1" title="${markEsc(item.title)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`:`<div class="videoFallback"><div><strong>Vídeo individual ainda não cadastrado.</strong><br><a href="${markEsc(item.learningExternal||'https://www.youtube.com/@mivcast')}" target="_blank" rel="noopener">Abrir canal MivCast</a></div></div>`;
 modal.classList.add('show');overlay.classList.add('show');
}
function closeLearningVideo(){document.getElementById('videoModal')?.classList.remove('show');document.getElementById('videoOverlay')?.classList.remove('show');const wrap=document.getElementById('videoFrameWrap');if(wrap)wrap.innerHTML=''}
async function loadLearningContent(){
 if(!mivSupabase)return;
 try{
  const cr=await mivSupabase.from('learning_content').select('*').eq('active',true).order('featured',{ascending:false}).order('sort_order');
  if(cr.error)throw cr.error;learningRows=cr.data||[];learningTracksRows=[];
  const existing=new Map(items.map(x=>[x.id,x]));
  learningRows.forEach(r=>{
   const data={id:'learn-'+r.slug,cat:r.area||'Aprenda & Aplique',format:r.format||'Conteúdo',access:r.access_level||'Grátis',price:r.price_label||'',icon:'◆',tag:r.area||'Aprendizado',title:r.title,desc:r.excerpt||'',img:r.image_url||imgs.learning,special:null,learningBody:r.body||'',learningExternal:r.external_url||'',learningVideoId:youtubeIdFromUrl(r.external_url),learningArea:r.area||'',learningLevel:r.level||'Todos',learningNiche:r.niche||'Todos',_learning:true};
   const target=existing.get(data.id);if(target)Object.assign(target,data);else{items.push(data);existing.set(data.id,data)}
  });
  const dynamicIds=learningRows.map(r=>'learn-'+r.slug);bases.learn.splice(0,bases.learn.length,...[...bases.learn.filter(id=>!String(id).startsWith('learn-')),...dynamicIds]);
  renderLearn();
 }catch(e){console.warn('[MIV learning]',e)}
}
function renderLearningTracks(){const el=document.getElementById('learningTracks');if(!el)return;el.innerHTML=learningTracksRows.slice(0,9).map(t=>`<article class="learningTrackCard"><small>TRILHA</small><h3>${t.title}</h3><p>${t.description||''}</p>${t.audience?`<p><b>Para:</b> ${t.audience}</p>`:''}</article>`).join('')}
function populateLearningAreas(){const sel=document.getElementById('learnArea');if(!sel)return;const current=sel.value;const areas=[...new Set(learningRows.map(x=>x.area).filter(Boolean))].sort();sel.innerHTML='<option value="Todos">Todas as áreas</option>'+areas.map(a=>`<option>${a}</option>`).join('');if([...sel.options].some(o=>o.value===current))sel.value=current}
async function loadCentralExtras(){
 if(!mivUser||!mivSupabase){centralExtras={ideas:[],purchases:[],progress:[]};return}
 try{const [a,b,c]=await Promise.all([
  mivSupabase.from('marketing_calendar_saved_ideas').select('*').eq('user_id',mivUser.id).order('created_at',{ascending:false}).limit(12),
  mivSupabase.from('user_purchases').select('item_id,item_type,status,amount_cents,provider,purchased_at').eq('user_id',mivUser.id).eq('status','paid').order('purchased_at',{ascending:false}).limit(30),
  mivSupabase.from('user_progress').select('progress_type,item_id,data,updated_at').eq('user_id',mivUser.id).order('updated_at',{ascending:false}).limit(50)
 ]);centralExtras={ideas:a.data||[],purchases:b.data||[],progress:c.data||[]}}catch(e){console.warn('[MIV central extras]',e)}
}
function renderCentralExtras(){
...
async function loadWorkspaceFromSupabase(){
 if(!mivUser||!mivSupabase)return;
 const localHistory=[...(state.history||[])].filter(Boolean),localReports=[...(state.reports||[])].filter(Boolean),localProgress=collectLocalProgress().filter(Boolean);
...
document.getElementById('learnSearch')?.addEventListener('input',renderLearn);document.getElementById('learnArea')?.addEventListener('change',renderLearn);document.getElementById('learnLevel')?.addEventListener('change',renderLearn);document.getElementById('centralAskMark')?.addEventListener('click',()=>{document.querySelector('.markFab')?.click();setTimeout(()=>{const ta=document.querySelector('#markForm textarea');if(ta){ta.value='Analise meu contexto, meu progresso e os itens desta Central. Quais devem ser meus 3 próximos passos prioritários e por quê?';ta.focus()}},150)});
initSupabase();
...
document.getElementById('backToReportAnalysis')?.addEventListener('click',backToReportAnalysis);
bindNicheSelectWithOther('regNiche');

document.getElementById('planFreeBtn')?.addEventListener('click',()=>{if(mivUser)route('central');else openAuth('register')});
document.getElementById('planAvulsoBtn')?.addEventListener('click',()=>{route('home');setTimeout(()=>document.getElementById('marketing')?.scrollIntoView({behavior:'smooth'}),80)});

/* V13.28 — assinaturas recorrentes */
const defaultSubscriptionPlans={pro:{plan_key:'pro',name:'Pro',price_cents:4790,currency_id:'BRL',frequency:1,frequency_type:'months'},premium:{plan_key:'premium',name:'Premium',price_cents:9790,currency_id:'BRL',frequency:1,frequency_type:'months'}};
let subscriptionPlans={...defaultSubscriptionPlans};
function normalizeSubscriptionPlan(row,plan){const base=defaultSubscriptionPlans[plan];return {...base,...(row||{}),price_cents:Number(row?.price_cents)>0?Number(row.price_cents):base.price_cents,currency_id:row?.currency_id||base.currency_id,frequency:Number(row?.frequency)>0?Number(row.frequency):base.frequency,frequency_type:row?.frequency_type||base.frequency_type}}
function moneyCents(c){return new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(Math.max(0,Number(c)||0)/100)+'/mês'}
function refreshSubscriptionPlanLabels(){const p=document.getElementById('planProPrice'),m=document.getElementById('planPremiumPrice'),cp=document.getElementById('centralProPrice'),cm=document.getElementById('centralPremiumPrice');if(p)p.textContent=moneyCents(subscriptionPlans.pro.price_cents);if(m)m.textContent=moneyCents(subscriptionPlans.premium.price_cents);if(cp)cp.textContent=moneyCents(subscriptionPlans.pro.price_cents);if(cm)cm.textContent=moneyCents(subscriptionPlans.premium.price_cents);try{renderSubscriptionManager()}catch(_){}}
async function loadSubscriptionPlans(){subscriptionPlans={...defaultSubscriptionPlans};if(!mivSupabase){refreshSubscriptionPlanLabels();return}try{const {data,error}=await mivSupabase.from('subscription_plans').select('*').eq('active',true).order('sort_order');if(error)throw error;(data||[]).forEach(x=>{if(defaultSubscriptionPlans[x.plan_key])subscriptionPlans[x.plan_key]=normalizeSubscriptionPlan(x,x.plan_key)});}catch(e){console.warn('[MIV plans]',e)}refreshSubscriptionPlanLabels()}
function subscriptionCouponCode(plan){const inputs=[...document.querySelectorAll(`[data-plan-coupon-input="${plan}"]`)];return String(inputs.map(i=>i.value).find(Boolean)||'').trim().toUpperCase()}
async function startSubscription(plan){if(!mivUser){openAuth('login');return}const cfg=normalizeSubscriptionPlan(subscriptionPlans[plan],plan);if(accessState.plan===plan){toast(`Este já é o seu plano ${plan==='premium'?'Premium':'PRO'}.`);return}if(accessState.plan==='premium'&&plan==='pro'){await scheduleDowngrade();return}const upgrading=accessState.plan==='pro'&&plan==='premium';const couponCode=subscriptionCouponCode(plan);const couponPart=couponCode?` usando o cupom ${couponCode}`:'';const msg=upgrading?`Fazer upgrade do PRO para Premium por ${moneyCents(cfg.price_cents)}${couponPart}? Seu PRO permanece ativo durante o checkout e a troca para Premium acontece após a confirmação do pagamento.`:`Assinar ${cfg.name} por ${moneyCents(cfg.price_cents)}${couponPart}?`;if(!confirm(msg))return;try{const {data:{session}}=await mivSupabase.auth.getSession();const r=await fetch('/api/create-subscription',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session?.access_token||''}`},body:JSON.stringify({plan,coupon_code:couponCode})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Não foi possível iniciar a assinatura.');if(d.coupon)toast(`Cupom ${d.coupon} aplicado.`);if(d.init_point)location.href=d.init_point;else{toast('Assinatura criada. Atualizando status...');setTimeout(()=>loadAccessFromSupabase(),1200)}}catch(e){alert(e.message||'Erro ao iniciar assinatura.')}}
...
async function scheduleDowngrade(){
 ...
 const cfg=normalizeSubscriptionPlan(subscriptionPlans.pro,'pro');
 const when=formatSubscriptionDate(accessState.subscription?.next_payment_date||accessState.subscription?.current_period_end);
...
setTimeout(()=>{loadSubscriptionPlans();['planProBtn','centralPlanProBtn'].forEach(id=>{const el=document.getElementById(id);if(el)el.onclick=()=>startSubscription('pro')});['planPremiumBtn','centralPlanPremiumBtn'].forEach(id=>{const el=document.getElementById(id);if(el)el.onclick=()=>startSubscription('premium')})},300);
