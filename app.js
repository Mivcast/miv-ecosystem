...
function marketCard(item,index,limit,lockedLabel){
 const saved=state.favorites.includes(item.id),locked=index>=limit,needed=index>=15?'Premium':'PRO';
 return `<article class="market-card ${locked?'locked':''}"><div class="market-card-top"><span class="market-stars" aria-label="${Number(item.importance)||0} de 5">${marketStars(item.importance)}</span><button class="heart ${saved?'saved':''}" ${locked?'data-scroll="planos"':`data-fav="${item.id}"`}>${locked?'🔒':saved?'♥':'♡'}</button></div><div class="market-card-body ${locked?'blurred':''}"><span class="meta-pill">${item.cat}</span><h3>${markEsc(item.title)}</h3><p>${markEsc(item.desc)}</p><strong>Estratégia do MARK para você</strong><p>${markEsc(item.mark_strategy)}</p></div>${item.source_url&&!locked?`<a class="source-link" href="${markEsc(item.source_url)}" target="_blank" rel="noopener">Fonte: ${markEsc(item.source||sourceHost(item.source_url))}</a>`:`<small class="source-link">${markEsc(item.source||'Fonte do radar')}</small>`}${locked?`<div class="market-lock"><b>${lockedLabel||`Libere no ${needed}`}</b><span>Assine para acessar mais cards do seu mercado.</span><button class="primary" data-scroll="planos">Ver planos</button></div>`:''}</article>`;
}
function renderMarketTrends(){const el=document.getElementById('trendsTrack');if(!el)return;if(!marketTrendItems.length){el.innerHTML=marketEmptyCard('trends','Clique em “Atualizar tendências” para buscar notícias recentes do seu nicho com fontes reais.',`Seu plano atual libera ${trendLimit()} notícia(s) por atualização.`, 'Atualizar tendências','refreshTrends');bindMarketEmptyActions();return}el.innerHTML=marketTrendItems.slice(0,30).map((x,i)=>marketCard(x,i,trendLimit(),i>=15?'Tendências Premium':'Mais tendências no PRO')).join('');bindCards();bindPlanScrolls()}
function renderCompetitorInsights(){const el=document.getElementById('competitorTrack');if(!el)return;const limit=competitorLimit(),cardLimit=limit*3;if(!competitorInsightItems.length){el.innerHTML=marketEmptyCard('competitors',`Digite até ${limit} concorrente(s) ou clique em “Sugerir concorrentes”.`,'O MARK procura movimentos públicos, posts, campanhas, ofertas e conteúdos que possam inspirar ações próprias para sua empresa.','Sugerir concorrentes','suggestCompetitors');bindMarketEmptyActions();return}el.innerHTML=competitorInsightItems.slice(0,30).map((x,i)=>marketCard(x,i,cardLimit,'Mais concorrentes nos planos')).join('');bindCards();bindPlanScrolls()}
...
async function refreshMarketTrends(){const btn=document.getElementById('refreshTrends'),limit=trendLimit(),track=document.getElementById('trendsTrack');if(!mivUser){pendingAuthAction={type:'refreshTrends'};marketTrendItems=[];localStorage.removeItem(MARKET_TRENDS_CACHE);if(track){track.innerHTML=marketLoginCard('trends');bindMarketEmptyActions()}openAuth('login');toast('Entre na sua conta para buscar tendências reais.');return}if(btn){btn.disabled=true;btn.textContent='Buscando notícias reais...'}if(track)track.innerHTML=marketLoadingCards(limit,'notícias e tendências');try{const token=await getAuthToken();const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:'trends',niche:state.profile.niche,limit})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar tendências agora.');marketTrendItems=(data.trends||[]).map((x,i)=>normalizeTrend(x,i)).filter(isLiveMarketItem).slice(0,limit);if(!marketTrendItems.length)throw new Error('A busca veio sem cards publicáveis. Tente novamente; o MARK vai ampliar a pesquisa por notícias, estudos e movimentos recentes do nicho.');saveMarketCache();renderMarketTrends();toast(`${marketTrendItems.length} tendência(s) para seu nicho.`)}catch(e){console.warn('[Market trends]',e);marketTrendItems=[];localStorage.removeItem(MARKET_TRENDS_CACHE);if(track){track.innerHTML=marketEmptyCard('trends','Não consegui montar tendências com fonte nesta tentativa.',e.message||'Tente atualizar novamente. A busca usa notícias, estudos e movimentos recentes do seu nicho, ampliando a janela quando necessário.','Tentar novamente','refreshTrends');bindMarketEmptyActions()}else renderMarketTrends();toast('A busca de tendências não concluiu. Veja o card da seção.')}finally{if(btn){btn.disabled=false;btn.textContent='Atualizar tendências'}}}
async function refreshCompetitors(suggest=false){let btn=document.getElementById(suggest?'suggestCompetitors':'refreshCompetitors'),track=document.getElementById('competitorTrack');if(!mivUser){pendingAuthAction={type:'refreshCompetitors',suggest};competitorInsightItems=[];localStorage.removeItem(MARKET_COMPETITORS_CACHE);if(track){track.innerHTML=marketLoginCard('competitors');bindMarketEmptyActions()}openAuth('login');toast('Entre na sua conta para buscar concorrentes reais.');return}try{let names=competitorNames(),limit=competitorLimit();if(!suggest&&!names.length){suggest=true;btn=document.getElementById('suggestCompetitors');toast('Vou sugerir referências do seu nicho para começar.')}if(btn){btn.disabled=true;btn.textContent=suggest?'Sugerindo...':'Buscando fontes...'}if(track)track.innerHTML=marketLoadingCards(limit*3,'movimentos de concorrentes');if(names.length>limit)toast(`Seu plano permite acompanhar ${limit} concorrente(s). Vou usar os primeiros da lista.`);const token=await getAuthToken();const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:suggest?'suggest_competitors':'competitors',niche:state.profile.niche,competitors:names.slice(0,limit),limit})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar concorrentes agora.');if(suggest&&Array.isArray(data.competitors)&&data.competitors.length){document.getElementById('competitorNames').value=data.competitors.slice(0,limit).join('\n')}competitorInsightItems=(data.cards||[]).map((x,i)=>normalizeCompetitor(x,i)).filter(isLiveMarketItem).slice(0,limit*3);if(!competitorInsightItems.length)throw new Error('A busca veio sem movimentos publicáveis. Tente nomes mais específicos ou use “Sugerir concorrentes” para o MARK encontrar referências do nicho.');saveMarketCache();renderCompetitorInsights();toast('Radar de concorrentes atualizado.')}catch(e){console.warn('[Competitors]',e);competitorInsightItems=[];localStorage.removeItem(MARKET_COMPETITORS_CACHE);if(track){track.innerHTML=marketEmptyCard('competitors','Não consegui montar movimentos com fonte nesta tentativa.',e.message||'Tente novamente com nomes mais específicos ou use “Sugerir concorrentes”.','Sugerir concorrentes','suggestCompetitors');bindMarketEmptyActions()}else renderCompetitorInsights();toast('A busca de concorrentes não concluiu. Veja o card da seção.')}finally{if(btn){btn.disabled=false;btn.textContent=suggest?'Sugerir concorrentes':'Buscar movimentos'}}}
...
function runPendingAuthAction(){
 ...
 if(action.type==='refreshCompetitors'){route('home');setTimeout(()=>{document.getElementById('concorrentes')?.scrollIntoView({behavior:'smooth',block:'start'});refreshCompetitors(!!action.suggest)},180);return true}
