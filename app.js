...
function isLiveMarketItem(x){return x&&x.source_url&&x.source&&x.source!=='MARK Radar'&&x.source!=='Análise orientativa'}
...
function bindMarketEmptyActions(){document.querySelectorAll('[data-click]').forEach(b=>b.onclick=()=>document.getElementById(b.dataset.click)?.click());document.querySelectorAll('[data-auth-market]').forEach(b=>b.onclick=()=>openAuth('login'))}
...
async function refreshMarketTrends(){const btn=document.getElementById('refreshTrends'),limit=trendLimit(),track=document.getElementById('trendsTrack');if(!mivUser){marketTrendItems=[];localStorage.removeItem(MARKET_TRENDS_CACHE);if(track){track.innerHTML=marketLoginCard('trends');bindMarketEmptyActions()}openAuth('login');toast('Entre na sua conta para buscar tendências reais.');return}if(btn){btn.disabled=true;btn.textContent='Buscando notícias reais...'}if(track)track.innerHTML=marketLoadingCards(limit,'notícias e tendências');try{const token=await getAuthToken();const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:'trends',niche:state.profile.niche,limit})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar tendências agora.');marketTrendItems=(data.trends||[]).map((x,i)=>normalizeTrend(x,i)).slice(0,limit);if(!marketTrendItems.length)throw new Error('A busca não retornou notícias com fonte real. Tente novamente em instantes.');saveMarketCache();renderMarketTrends();toast(`${marketTrendItems.length} tendência(s) com fonte real para seu nicho.`)}catch(e){console.warn('[Market trends]',e);marketTrendItems=[];localStorage.removeItem(MARKET_TRENDS_CACHE);renderMarketTrends();toast(e.message||'Não consegui buscar tendências reais agora.')}finally{if(btn){btn.disabled=false;btn.textContent='Atualizar tendências'}}}
async function refreshCompetitors(suggest=false){const btn=document.getElementById(suggest?'suggestCompetitors':'refreshCompetitors'),track=document.getElementById('competitorTrack');if(!mivUser){competitorInsightItems=[];localStorage.removeItem(MARKET_COMPETITORS_CACHE);if(track){track.innerHTML=marketLoginCard('competitors');bindMarketEmptyActions()}openAuth('login');toast('Entre na sua conta para buscar concorrentes reais.');return}if(btn){btn.disabled=true;btn.textContent=suggest?'Sugerindo...':'Buscando fontes...'}try{const names=competitorNames(),limit=competitorLimit();if(!suggest&&!names.length)throw new Error('Digite pelo menos um concorrente ou use “Sugerir concorrentes”.');if(track)track.innerHTML=marketLoadingCards(limit*3,'movimentos de concorrentes');if(names.length>limit)toast(`Seu plano permite acompanhar ${limit} concorrente(s). Vou usar os primeiros da lista.`);const token=await getAuthToken();const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:suggest?'suggest_competitors':'competitors',niche:state.profile.niche,competitors:names.slice(0,limit),limit})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar concorrentes agora.');if(suggest&&Array.isArray(data.competitors)&&data.competitors.length){document.getElementById('competitorNames').value=data.competitors.slice(0,limit).join('\n')}competitorInsightItems=(data.cards||[]).map((x,i)=>normalizeCompetitor(x,i)).slice(0,limit*3);if(!competitorInsightItems.length)throw new Error('A busca não retornou movimentos públicos com fonte real. Tente outros nomes ou use “Sugerir concorrentes”.');saveMarketCache();renderCompetitorInsights();toast('Radar de concorrentes atualizado com fontes reais.')}catch(e){console.warn('[Competitors]',e);competitorInsightItems=[];localStorage.removeItem(MARKET_COMPETITORS_CACHE);renderCompetitorInsights();toast(e.message||'Não consegui buscar concorrentes reais agora.')}finally{if(btn){btn.disabled=false;btn.textContent=suggest?'Sugerir concorrentes':'Buscar movimentos'}}}
...
function openPaywall(item){state.current=item;document.getElementById('payTitle').textContent=item.title;document.getElementById('singlePrice').textContent=item.price||'Plano Pro';const cc=document.getElementById('couponCode');if(cc)cc.value='';const cf=document.getElementById('couponFeedback');if(cf){cf.style.display='none';cf.textContent=''};const bs=document.getElementById('buySingle');if(bs)bs.childNodes[0].textContent='Comprar este item ';document.getElementById('paywall').classList.add('show');document.getElementById('overlay').classList.add('show')}
...
async function applyCouponPreview(){
 ...
 if(!code){if(feedback){feedback.style.display='block';feedback.textContent='Digite um cupom.'}return}if(!mivUser||!mivSupabase){closePaywall();openAuth('login');toast('Entre na sua conta para aplicar o cupom.');return}
...
async function startMercadoPagoSinglePurchase(){
 ...
 if(!item)return;
 if(!mivUser||!mivSupabase){closePaywall();openAuth('login');toast('Entre na sua conta para comprar.');return}
