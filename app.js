...
function marketCard(item,index,limit,lockedLabel){
 ...
 const actions=!locked&&item.special==='competitor-placeholder'?`<button class="primary market-card-action" data-competitor-channel="${markEsc(item.channel||item.title)}">Analisar ${markEsc(item.title)}</button>`:!locked&&item.special==='competitor-insight'?`<div class="market-card-actions"><button class="primary market-card-action compact" data-competitor-channel="${markEsc(item.channel||item.title)}">Atualizar este canal</button><button class="outline market-card-action compact" data-market-mark="${markEsc(item.title)}">Sugestões do MARK</button></div>`:'';
...
async function refreshCompetitors(suggest=false,channel=''){
 ...
 if(!mivUser){
  pendingAuthAction={type:'refreshCompetitors',suggest,channel};
 ...
 try{
  let names=competitorNames(),limit=competitorLimit();
  if(!suggest&&!names.length)throw new Error('Digite concorrentes ou clique em “Sugerir concorrentes” antes de analisar os canais.');
  if(btn){btn.disabled=true;btn.textContent=suggest?'Sugerindo concorrentes...':channel?`Atualizando ${channel}...`:'Atualizando canais...'}
  setCompetitorStatus(suggest?'Aguarde até 1 minutinho: o MARK está pesquisando concorrentes reais, ativos e úteis para inspiração no seu nicho.':channel?`Investigando sinais públicos em ${channel} para os concorrentes salvos.`:'Investigando todos os canais dos concorrentes salvos. Isso pode levar alguns segundos.')
  if(!suggest&&!channel&&track)track.innerHTML=marketLoadingCards(7,'canais dos concorrentes');
  if(!suggest&&channel)toast(`Atualizando apenas ${channel} para os concorrentes salvos.`);
  ...
  const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:suggest?'suggest_competitors':'competitors',niche:state.profile.niche,competitors:names,limit,channel})});
  ...
  if(!nextCards.length)throw new Error('A busca veio sem canais publicáveis. Tente nomes mais específicos ou use “Sugerir concorrentes” para o MARK encontrar referências do nicho.');
  if(channel&&competitorInsightItems.length){
   const target=assetSlug(channel);
   const replacement=nextCards.find(x=>assetSlug(x.channel||x.title)===target);
   if(replacement)competitorInsightItems=competitorInsightItems.map(x=>assetSlug(x.channel||x.title)===target?replacement:x);
   else competitorInsightItems=nextCards;
  }else{
   competitorInsightItems=nextCards;
  }
  saveMarketCache();
  renderCompetitorInsights();
  setCompetitorStatus(channel?`${channel} atualizado para os concorrentes no radar.`:'Todos os canais foram atualizados para os concorrentes no radar.');
  toast(channel?`${channel} atualizado.`:'Todos os canais dos concorrentes foram atualizados.');
 }catch(e){
  ...
  if(refreshBtn){refreshBtn.disabled=false;refreshBtn.textContent='Atualizar todos os canais'}
 }
}
