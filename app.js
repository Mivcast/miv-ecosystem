...
async function startMercadoPagoSinglePurchase(){
 ...
 try{
  ...
  const r=await fetch('/api/create-mercadopago-preference',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({item_id:normalizeAccessItemId(item.id)})});
  const data=await r.json().catch(()=>({}));
  if(!r.ok)throw new Error(data.error||'Não foi possível abrir o pagamento.');
  if(!data.checkout_url)throw new Error('URL do checkout não recebida.');
