...
function saveMarketCache(){localStorage.setItem('mivMarketTrendItems',JSON.stringify(marketTrendItems));localStorage.setItem('mivCompetitorInsightItems',JSON.stringify(competitorInsightItems))}
function hydrateMarketCache(){try{marketTrendItems=JSON.parse(localStorage.getItem('mivMarketTrendItems')||'[]')||[]}catch(e){marketTrendItems=[]}try{competitorInsightItems=JSON.parse(localStorage.getItem('mivCompetitorInsightItems')||'[]')||[]}catch(e){competitorInsightItems=[]}}
function marketStars(n){const qty=Math.max(0,Math.min(5,Math.round(Number(n)||0)));return `${'★'.repeat(qty)}${'☆'.repeat(5-qty)}`}
function fallbackTrends(niche=state.profile.niche){
 const topics=['Busca local e Google','Conteúdo educativo','WhatsApp e atendimento','Prova social','Vídeos curtos','Experiência do cliente','Automação simples','Oferta e diferenciação','Datas comerciais','Comunidade','Parcerias locais','Recompra','Anúncios com intenção','Autoridade profissional','IA aplicada','Avaliações online','Jornada de compra','Ticket médio','Retenção','Pesquisa de preço','Marca visual','Atendimento rápido','Conteúdo sazonal','Página de conversão','Indicação','Relacionamento','Bastidores','Produtos digitais','Eventos locais','Métricas comerciais'];
 return topics.map((t,i)=>({id:`trend-${assetSlug(niche)}-${i+1}`,cat:'Tendências',format:'Radar de mercado',access:'Grátis',icon:'★',tag:'MERCADO',title:`${t} em alta para ${niche}`,desc:`Acompanhe como ${t.toLowerCase()} pode influenciar a decisão dos clientes no seu nicho.`,mark_strategy:`Use este sinal para criar um conteúdo, oferta ou melhoria simples. Comece pequeno: uma publicação, uma conversa no WhatsApp ou uma revisão do seu processo atual.`,importance:Math.max(3,5-(i%3)),source:'MARK Radar',source_url:'',special:'market-trend',img:'assets/cards/analise-mercado.jpg'}));
}
function fallbackCompetitors(names=[]){
 const list=names.length?names:['Concorrente local','Referência do nicho','Empresa parecida'];
 const cards=[];
 list.forEach((name,idx)=>['posicionamento','conteúdo','oferta'].forEach((kind,j)=>cards.push({id:`competitor-${assetSlug(name)}-${kind}`,cat:'Concorrentes',format:'Monitoramento',access:'Grátis',icon:'⌕',tag:String(name).toUpperCase().slice(0,22),title:`O que observar em ${name}: ${kind}`,desc:`Veja como esse concorrente comunica ${kind} e compare com a experiência que sua empresa entrega hoje.`,mark_strategy:`Anote uma ideia útil, adapte ao seu contexto e transforme em uma ação própria. A intenção é se inspirar, não copiar literalmente.`,importance:idx===0?4:3,source:'Análise orientativa',source_url:'',special:'competitor-insight',img:'assets/cards/analise-mercado.jpg'})));
...
function renderMarketTrends(){const el=document.getElementById('trendsTrack');if(!el)return;if(!marketTrendItems.length)marketTrendItems=fallbackTrends();el.innerHTML=marketTrendItems.slice(0,30).map((x,i)=>marketCard(x,i,trendLimit(),i>=15?'Tendências Premium':'Mais tendências no PRO')).join('');bindCards();bindPlanScrolls()}
function renderCompetitorInsights(){const el=document.getElementById('competitorTrack');if(!el)return;const names=competitorNames().slice(0,competitorLimit());if(!competitorInsightItems.length)competitorInsightItems=fallbackCompetitors(names);el.innerHTML=competitorInsightItems.slice(0,30).map((x,i)=>marketCard(x,i,30,'Mais concorrentes nos planos')).join('');bindCards();bindPlanScrolls()}
...
async function refreshMarketTrends(){const btn=document.getElementById('refreshTrends');if(btn){btn.disabled=true;btn.textContent='Buscando tendências...'}try{const token=await getAuthToken();const r=await fetch('/api/market-intel',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:'trends',niche:state.profile.niche,limit:30})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar tendências agora.');marketTrendItems=(data.trends||[]).map((x,i)=>normalizeTrend(x,i)).slice(0,30);if(!marketTrendItems.length)throw new Error('Nenhuma tendência retornada.');saveMarketCache();renderMarketTrends();toast('Tendências atualizadas para seu nicho.')}catch(e){console.warn('[Market trends]',e);if(!marketTrendItems.length)marketTrendItems=fallbackTrends();renderMarketTrends();toast(e.message||'Usei tendências guia até a busca responder.')}finally{if(btn){btn.disabled=false;btn.textContent='Atualizar tendências'}}}
async function refreshCompetitors(suggest=false){const btn=document.getElementById(suggest?'suggestCompetitors':'refreshCompetitors');if(btn){btn.disabled=true;btn.textContent=suggest?'Sugerindo...':'Buscando...'}try{const names=competitorNames(),limit=competitorLimit();if(!suggest&&!names.length)throw new Error('Digite pelo menos um concorrente ou use “Sugerir concorrentes”.');if(names.length>limit)toast(`Seu plano permite acompanhar ${limit} concorrente(s). Vou usar os primeiros da lista.`);const token=await getAuthToken();const r=await fetch('/api/market-intel',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:suggest?'suggest_competitors':'competitors',niche:state.profile.niche,competitors:names.slice(0,limit),limit})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar concorrentes agora.');if(suggest&&Array.isArray(data.competitors)&&data.competitors.length){document.getElementById('competitorNames').value=data.competitors.slice(0,limit).join('\n')}competitorInsightItems=(data.cards||[]).map((x,i)=>normalizeCompetitor(x,i)).slice(0,30);if(!competitorInsightItems.length)throw new Error('Nenhum movimento retornado.');saveMarketCache();renderCompetitorInsights();toast('Radar de concorrentes atualizado.')}catch(e){console.warn('[Competitors]',e);competitorInsightItems=fallbackCompetitors(competitorNames().slice(0,competitorLimit()));renderCompetitorInsights();toast(e.message||'Montei um radar orientativo para começar.')}finally{if(btn){btn.disabled=false;btn.textContent=suggest?'Sugerir concorrentes':'Buscar movimentos'}}}
