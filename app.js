...
function hydrateMarketCache(){try{marketTrendItems=(JSON.parse(localStorage.getItem('mivMarketTrendItems')||'[]')||[]).filter(isLiveMarketItem)}catch(e){marketTrendItems=[]}try{competitorInsightItems=(JSON.parse(localStorage.getItem('mivCompetitorInsightItems')||'[]')||[]).filter(isLiveMarketItem)}catch(e){competitorInsightItems=[]}if(!marketTrendItems.length)localStorage.removeItem('mivMarketTrendItems');if(!competitorInsightItems.length)localStorage.removeItem('mivCompetitorInsightItems')}
...
function bindMarketEmptyActions(){document.querySelectorAll('[data-click]').forEach(b=>b.onclick=()=>document.getElementById(b.dataset.click)?.click())}
