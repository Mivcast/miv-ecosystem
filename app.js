...
function isLiveMarketItem(x){return x&&x.source&&x.source!=='MARK Radar'&&x.source!=='Análise orientativa'}
...
function normalizeCompetitor(raw,i){const item={id:raw.id||`competitor-${assetSlug(raw.competitor||'mercado')}-${i+1}`,cat:'Concorrentes',format:'Monitoramento',access:'Grátis',icon:'⌕',tag:String(raw.competitor||'CONCORRENTE').toUpperCase().slice(0,22),title:raw.title||`Movimento de ${raw.competitor||'concorrente'}`,desc:raw.description||raw.desc||'Sinal observado para comparar com o seu posicionamento.',mark_strategy:raw.mark_strategy||raw.strategy||'Adapte a ideia ao seu negócio e transforme em uma melhoria própria.',importance:raw.importance||3,source:raw.source||sourceHost(raw.source_url,'Fonte'),source_url:raw.source_url||raw.url||'',special:'competitor-insight',img:'assets/cards/analise-mercado.jpg'};if(!item.source_url&&item.source)item.source_url=sourceSearchUrl(item);return item}
...
function competitorChannelPlaceholder(name,i){return {id:`competitor-channel-${assetSlug(name)}`,cat:'Concorrentes',format:'Canal',access:'Grátis',icon:'⌕',tag:'RADAR',title:name,desc:i===0?'Clique para analisar o Instagram dos concorrentes informados ou sugeridos.':'Este canal fica visível para você saber o que existe no radar, mas a análise completa é liberada nos planos pagos.',mark_strategy:'Digite concorrentes ou use “Sugerir concorrentes”. Depois clique no canal para o MARK buscar sinais públicos e transformar em ideias práticas.',importance:i<4?5:4,source:'Radar por canal',source_url:'',special:'competitor-placeholder',channel:name,img:'assets/cards/analise-mercado.jpg'}}
...
function marketCard(item,index,limit,lockedLabel){
 const saved=state.favorites.includes(item.id),locked=index>=limit,needed=index>=15?'Premium':'PRO',isCompetitor=item.special==='competitor-insight'||item.special==='competitor-placeholder',channelClass=isCompetitor?`competitor-channel-card channel-${assetSlug(item.channel||item.title)}`:'',bodyClass=locked&&isCompetitor?'':locked?'blurred':'',descHtml=isCompetitor?competitorDescriptionHtml(item.desc):markEsc(item.desc),actions=!locked&&item.special==='competitor-placeholder'?`<button class="primary market-card-action" data-competitor-channel="${markEsc(item.channel||item.title)}">Analisar ${markEsc(item.title)}</button>`:!locked&&item.special==='competitor-insight'?`<button class="outline market-card-action compact" data-market-mark="${markEsc(item.title)}">Sugestões do MARK</button>`:'';
...
function loadingTrendPlaceholder(i){return {id:`loading-trend-${i}`,cat:'Tendências',format:'Pesquisando',access:'Grátis',icon:'★',tag:'BUSCA',title:'Completando tendências do seu plano...',desc:'O MARK está buscando mais fontes reais para liberar a quantidade correta do seu plano.',mark_strategy:'Aguarde um instante. Assim que novas notícias forem encontradas, este espaço será substituído por cards com fonte clicável.',importance:4,source:'Buscando fontes reais',source_url:'',special:'market-loading-placeholder',img:'assets/cards/analise-mercado.jpg'}}
...
function renderMarketTrends(){const el=document.getElementById('trendsTrack');if(!el)return;if(!marketTrendItems.length){el.innerHTML=marketEmptyCard('trends','Clique em “Atualizar tendências” para buscar notícias recentes do seu nicho com fontes reais.',`Seu plano atual libera ${trendLimit()} notícia(s) por atualização.`, 'Atualizar tendências','refreshTrends');bindMarketEmptyActions();return}const limit=trendLimit(),real=marketTrendItems.slice(0,limit),filling=mivUser&&accessState.ready&&real.length<limit?Array.from({length:limit-real.length},(_,i)=>loadingTrendPlaceholder(real.length+i)):[],locked=hasPremiumAccess()?[]:Array.from({length:Math.max(0,30-real.length-filling.length)},(_,i)=>lockedMarketPlaceholder(real.length+filling.length+i,'trend'));el.innerHTML=[...real,...filling,...locked].map((x,i)=>marketCard(x,i,limit,i>=15?'Tendências Premium':'Tendências PRO')).join('');bindCards();bindPlanScrolls();ensureMarketTrendCapacity(limit)}
...
function saveCompetitorNames(names=competitorNames()){localStorage.setItem(MARKET_COMPETITOR_NAMES_CACHE,JSON.stringify([...new Set((names||[]).map(x=>String(x).trim()).filter(Boolean))]))}
function hydrateCompetitorNames(){const el=document.getElementById('competitorNames');if(!el||el.value.trim())return;try{const names=JSON.parse(localStorage.getItem(MARKET_COMPETITOR_NAMES_CACHE)||'[]');if(Array.isArray(names)&&names.length)el.value=names.join('\n')}catch(e){}}
...
async function refreshCompetitors(suggest=false,channel=''){let btn=document.getElementById('suggestCompetitors'),track=document.getElementById('competitorTrack');if(!mivUser){pendingAuthAction={type:'refreshCompetitors',suggest,channel};competitorInsightItems=[];localStorage.removeItem(MARKET_COMPETITORS_CACHE);if(track){track.innerHTML=marketLoginCard('competitors');bindMarketEmptyActions()}openAuth('login');toast('Entre na sua conta para buscar concorrentes reais.');return}if(!accessState.ready)await loadAccessFromSupabase();try{let names=competitorNames(),limit=competitorLimit();if(!suggest&&!names.length)throw new Error('Digite os concorrentes ou clique em “Sugerir concorrentes” antes de analisar um canal.');if(btn&&suggest){btn.disabled=true;btn.textContent='Sugerindo...'}if(!suggest&&track)track.innerHTML=marketLoadingCards(7,channel?`${channel} dos concorrentes`:'canais dos concorrentes');if(names.length>limit)toast(`Seu plano permite acompanhar ${limit} concorrente(s). Vou usar os primeiros da lista.`);names=names.slice(0,limit);if(names.length)saveCompetitorNames(names);const token=await getAuthToken();const r=await fetch('/api/mark-ai',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:suggest?'suggest_competitors':'competitors',niche:state.profile.niche,competitors:names,limit,channel})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Não foi possível buscar concorrentes agora.');if(suggest){const suggested=(Array.isArray(data.competitors)?data.competitors:[]).slice(0,limit);if(suggested.length){document.getElementById('competitorNames').value=suggested.join('\n');saveCompetitorNames(suggested)}competitorInsightItems=[];localStorage.removeItem(MARKET_COMPETITORS_CACHE);renderCompetitorInsights();toast(suggested.length?'Concorrentes sugeridos e salvos. Agora clique no canal que deseja analisar.':'Não encontrei sugestões claras; digite nomes para analisar.');return}competitorInsightItems=(data.cards||[]).map((x,i)=>normalizeCompetitor(x,i)).filter(isLiveMarketItem).slice(0,7);if(!competitorInsightItems.length)throw new Error('A busca veio sem canais publicáveis. Tente nomes mais específicos ou use “Sugerir concorrentes” para o MARK encontrar referências do nicho.');saveMarketCache();renderCompetitorInsights();toast(channel?`${channel} e outros canais atualizados.`:'Canais dos concorrentes atualizados.')}catch(e){console.warn('[Competitors]',e);toast(e.message||'A busca de concorrentes não concluiu.');if(!competitorInsightItems.length)renderCompetitorInsights()}finally{if(btn){btn.disabled=false;btn.textContent='Sugerir concorrentes'}}}
...
function runPendingAuthAction(){
 ...
 if(action.type==='refreshCompetitors'){route('home');setTimeout(()=>{document.getElementById('concorrentes')?.scrollIntoView({behavior:'smooth',block:'start'});refreshCompetitors(!!action.suggest,action.channel||'')},180);return true}
...
async function loadLearningContent(){
 ...
 try{
  const [cr,tr]=await Promise.all([
   mivSupabase.from('learning_content').select('*').eq('active',true).order('featured',{ascending:false}).order('sort_order'),
   mivSupabase.from('learning_tracks').select('*').eq('active',true).order('sort_order')
  ]);
  if(cr.error)throw cr.error;if(tr.error)throw tr.error;learningRows=cr.data||[];learningTracksRows=tr.data||[];
  ...
  renderLearn();renderLearningTracks();
