...
async function openCalendar(){const modal=document.getElementById('calendarModal');modal.classList.add('open');calDate=new Date();fillCalSelectors();const local=readCalendarPrefs();document.getElementById('calNiche').value=local.niche||state.profile.niche||'';document.getElementById('calCity').value=local.city||state.profile.city||'';document.getElementById('calCustomDates').value=local.customText||'';calDynamicEvents=Array.isArray(local.dynamicEvents)?local.dynamicEvents:[];calDynamicSources=Array.isArray(local.dynamicSources)?local.dynamicSources:[];if(mivUser&&mivSupabase){try{const {data}=await mivSupabase.from('marketing_calendar_profiles').select('*').eq('user_id',mivUser.id).maybeSingle();if(data){document.getElementById('calNiche').value=data.niche||document.getElementById('calNiche').value;document.getElementById('calCity').value=data.city||document.getElementById('calCity').value;document.getElementById('calCustomDates').value=data.custom_text||'';calDynamicEvents=Array.isArray(data.dynamic_events)?data.dynamic_events:[];calDynamicSources=Array.isArray(data.dynamic_sources)?data.dynamic_sources:[];saveCalendarPrefs({niche:data.niche||'',city:data.city||'',customText:data.custom_text||'',customEvents:Array.isArray(data.custom_events)?data.custom_events:[],dynamicEvents:calDynamicEvents,dynamicSources:calDynamicSources,savedAt:data.updated_at,dynamicUpdatedAt:data.dynamic_updated_at})}}catch(e){console.warn('[Calendar cloud load]',e)}}renderCalendar();updateCalSavedStatus();renderCalSources()}
