...
module.exports=async function(req,res){
 ...
 try{
  ...
  const paymentEnv=validatePaymentEnvironment(mp);
  if(!paymentEnv.ok){console.error('[MIV payment environment]',paymentEnv);return send(res,503,{error:paymentEnv.error,code:'payment_environment_mismatch'});}
  ...
  const isTestToken=paymentEnv.effective==='test';
  ...
  return send(res,200,{ok:true,id:data.id,status:data.status,init_point:checkoutUrl(data,paymentEnv.effective),upgrade:isUpgrade,payment_environment:paymentEnv.effective});
