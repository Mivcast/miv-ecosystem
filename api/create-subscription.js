function paymentEnvironment(token){
 const configured=String(process.env.MIV_PAYMENT_MODE||'test').trim().toLowerCase();
 const effective=/^TEST-/i.test(String(token||'').trim())?'test':'production';
 if(!['test','production'].includes(configured)) return {ok:false,effective,error:'MIV_PAYMENT_MODE inválido.'};
 if(configured!==effective) return {ok:false,effective,error:`Ambiente de pagamento incompatível: sistema=${configured}, credencial=${effective}.`};
 return {ok:true,effective};
}
function siteUrl(req){
 const explicit=String(process.env.MIV_SITE_URL||'').trim().replace(/\/$/,'');
 if(explicit) return explicit;
 const host=req.headers['x-forwarded-host']||req.headers.host||'miv-ecosystem.vercel.app';
 const proto=req.headers['x-forwarded-proto']||'https';
 return `${proto}://${host}`;
}
function mpCheckout(data,env){return env==='test'?(data.sandbox_init_point||data.init_point||null):(data.init_point||null)}
...
module.exports=async function(req,res){
 ...
 try{
  ...
  const isTestToken=payEnv.effective==='test';
  ...
  return send(res,200,{ok:true,id:data.id,status:data.status,init_point:mpCheckout(data,payEnv.effective),upgrade:isUpgrade});
