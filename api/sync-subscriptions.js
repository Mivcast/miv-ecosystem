...
function subscriptionCouponRef(externalReference){
 const parts=String(externalReference||'').split('|');const out={};
 for(const part of parts){const i=part.indexOf(':');if(i>0)out[part.slice(0,i)]=part.slice(i+1)}
 return out.coupon?{couponId:out.coupon,discountCents:Number(out.discount||0)}:null;
}
async function recordSubscriptionCoupon(url,key,userId,plan,sub){
 const ref=subscriptionCouponRef(sub.external_reference);if(!ref?.couponId)return;
 const itemId=`plan:${plan}`;
 const existing=await sbFetch(url,key,`coupon_redemptions?coupon_id=eq.${encodeURIComponent(ref.couponId)}&user_id=eq.${encodeURIComponent(userId)}&item_id=eq.${encodeURIComponent(itemId)}&select=id&limit=1`);
 if(existing?.length)return;
 await sbFetch(url,key,'coupon_redemptions',{method:'POST',body:JSON.stringify({coupon_id:ref.couponId,user_id:userId,item_id:itemId,discount_cents:ref.discountCents,redeemed_at:new Date().toISOString()})});
}
 ...
 try{
  ...
  for(const row of (subs||[])){
   ...
   try{
    ...
    if(status==='active')await recordSubscriptionCoupon(su,sk,row.user_id,row.plan,sub);
