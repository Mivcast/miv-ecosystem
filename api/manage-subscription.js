...
module.exports=async function(req,res){
 ...
 try{
  ...
  if(action==='schedule_downgrade'){
   ...
   const plans=await sb(su,sk,'subscription_plans?plan_key=eq.pro&active=eq.true&select=*&limit=1');const pro=plans?.[0];if(!pro||Number(pro.price_cents)<=0)return send(res,409,{error:'Preço do PRO não configurado.'});
  ...
  if(action==='undo_downgrade'){
   ...
   const plans=await sb(su,sk,'subscription_plans?plan_key=eq.premium&active=eq.true&select=*&limit=1');const premium=plans?.[0];if(!premium||Number(premium.price_cents)<=0)return send(res,409,{error:'Preço do Premium não configurado.'});
