...
function cleanMarketCard(x){return {competitor:clean(x.competitor,120),channel:clean(x.channel||x.title,80),title:clean(x.title,180),description:clean(x.description,1300),mark_strategy:clean(x.mark_strategy||x.strategy,950),importance:Math.max(0,Math.min(5,Number(x.importance)||3)),source:clean(x.source,140),source_url:clean(x.source_url||x.url,600)}}
...
function movementFromSignals(channel,niche,signals){
  ...
  const base={
    instagram:[`☐ Posicionamento claro na bio — deixar evidente especialidade, localização, promessa principal e caminho para contato.`,`☐ Reels educativos — responder dúvidas frequentes em 20 a 60 segundos para ganhar descoberta.`,`☐ Carrosséis salváveis — transformar sinais, erros, mitos e checklists em posts úteis para salvar.`],
    facebook:[`☐ Presença local consistente — publicar avisos, bastidores, provas e conteúdos de relacionamento.`,`☐ Comunidade e confiança — estimular comentários, mensagens e compartilhamentos em grupos ou página.`,`☐ Reaproveitamento inteligente — adaptar os melhores conteúdos do Instagram para públicos locais.`],
    tiktok:[`☐ Ganchos rápidos — começar com sintomas, dúvidas, mitos ou situações cotidianas do público.`,`☐ Bastidores espontâneos — mostrar rotina, processo e opinião sem depender de artes formais.`,`☐ Quadros repetíveis — criar séries curtas para o público reconhecer e voltar.`],
    'google-business':[`☐ Perfil atualizado — manter serviços, descrição, horários, fotos e links sem atrito.`,`☐ Prova social visível — estimular e responder avaliações com cuidado e linguagem do nicho.`,`☐ Posts rápidos de decisão — publicar novidades e diferenciais que ajudem quem está pesquisando agora.`],
    youtube:[`☐ Autoridade aprofundada — explicar temas complexos em vídeos mais completos e pesquisáveis.`,`☐ Cortes reaproveitáveis — transformar vídeos longos em shorts, Reels e carrosséis.`,`☐ Séries por dor do cliente — organizar playlists e quadros por dúvidas reais.`],
    'site-landing-page':[`☐ Página por intenção — explicar serviços, diferenciais, provas e próximos passos sem depender só da rede social.`,`☐ Conteúdo pesquisável — usar artigos ou FAQs para dúvidas que o público busca no Google.`,`☐ Conversão clara — facilitar contato, WhatsApp, agendamento ou compra com botões visíveis.`],
    marketplaces:[`☐ Oferta fácil de comparar — destacar benefício, prova, prazo, preço e diferenciais logo no início.`,`☐ Fotos e descrições fortes — reduzir dúvidas antes da compra com detalhes objetivos.`,`☐ Reputação como argumento — usar avaliações e perguntas respondidas para aumentar confiança.`]
  };
