...
async function handleMarketIntel(req,res,{su,sk,gk,user}){
  ...
  if(action==='trends'){
    ...
    const prompt=`Hoje é ${today}. Pesquise na web tendências, notícias, mudanças de comportamento, tecnologia, regulação, consumo, marketing e oportunidades atuais no Brasil que sejam úteis para o nicho abaixo. Priorize fontes confiáveis, recentes e com potencial de virar conteúdo, campanha, atendimento, oferta ou decisão estratégica.\n\n${marketCompanyText(company,profile,niche)}\n\nRetorne SOMENTE JSON válido no formato {"trends":[{"title":"...","description":"...","mark_strategy":"...","importance":0-5,"source":"nome do site","source_url":"https://..."}]}.\nCrie exatamente ${limit} cards. Em cada card, escreva:\n- title: título claro e curto da tendência.\n- description: o que aconteceu, quando aconteceu se a fonte permitir, e por que isso importa para o nicho.\n- mark_strategy: uma dica prática no estilo "Estratégia do MARK para você", explicando como usar a tendência em post, WhatsApp, campanha, oferta, conteúdo educativo, relacionamento ou posicionamento.\n- importance: nota de 0 a 5 pela relevância para o nicho.\n- source e source_url: fonte original confiável.\nNão invente cura, lei, número, notícia, pesquisa ou fato específico sem fonte. Se a relação com o nicho for indireta, explique de forma honesta como usar sem forçar promoção.`;
  ...
  if(action==='competitors'||action==='suggest_competitors'){
    ...
    const prompt=action==='suggest_competitors'
      ?`Hoje é ${today}. Pesquise referências e concorrentes prováveis no Brasil para o nicho abaixo. Sugira até ${max} nomes que uma empresa poderia acompanhar e gere movimentos observáveis, ideias de conteúdo, oferta, presença digital, posicionamento, atendimento, produto ou experiência.\n\n${marketCompanyText(company,profile,niche)}\n\nRetorne SOMENTE JSON válido: {"competitors":["..."],"cards":[{"competitor":"...","title":"...","description":"...","mark_strategy":"...","importance":0-5,"source":"nome do site","source_url":"https://..."}]}. Gere até 3 cards por concorrente. A ideia é inspirar com inteligência, sem orientar cópia literal.`
      :`Hoje é ${today}. Pesquise na web novidades e movimentos públicos dos concorrentes abaixo para inspirar uma empresa do nicho informado. Observe lançamentos, posts, campanhas, canais, posicionamento, prova social, parcerias, eventos, conteúdos e ofertas. Não invente ações específicas sem fonte; se não encontrar fato recente, transforme em observação estratégica segura e diga o que pode ser adaptado sem copiar literalmente.\n\n${marketCompanyText(company,profile,niche)}\nConcorrentes: ${names.length?names.join(', '):'não informados'}\n\nRetorne SOMENTE JSON válido: {"cards":[{"competitor":"...","title":"...","description":"...","mark_strategy":"...","importance":0-5,"source":"nome do site","source_url":"https://..."}]}. Gere até 3 cards por concorrente.`;
    ...
    const cards=(Array.isArray(obj.cards)?obj.cards:[]).map(cleanMarketCard).filter(x=>x.title&&x.description).slice(0,max*3);
