...
function channelStrategy(channel,niche){
  const map={
    instagram:`Como eu aplicaria isso: escolha 3 pilares para o Instagram de ${niche}: identificação, educação e autoridade. Grave Reels curtos com perguntas reais, transforme os melhores temas em carrosséis salváveis e use Stories para bastidores, enquete e convite para conversa.`,
    facebook:`Como eu aplicaria isso: use o Facebook como presença local e relacionamento. Reaproveite conteúdos fortes, publique provas e avisos úteis, responda comentários e direcione conversas qualificadas para WhatsApp ou atendimento.`,
    tiktok:`Como eu aplicaria isso: crie uma série simples com ganchos de 20 a 40 segundos. Comece por mitos, erros comuns e situações do dia a dia, mantendo linguagem leve e repetição de quadro para ganhar reconhecimento.`,
    'google-business':`Como eu aplicaria isso: trate o Google Empresas como vitrine de decisão. Atualize fotos, serviços e descrição, publique novidades curtas e responda avaliações de forma cuidadosa para aumentar confiança local.`,
    youtube:`Como eu aplicaria isso: transforme dúvidas mais densas em vídeos explicativos e depois corte os melhores trechos para Reels, Shorts e carrosséis. Organize tudo por temas que o cliente realmente pesquisa.`,
    'site-landing-page':`Como eu aplicaria isso: crie páginas que respondam às dúvidas antes do contato. Use promessa clara, provas, FAQ e CTA visível, conectando os conteúdos das redes para uma página que converte melhor.`,
    marketplaces:`Como eu aplicaria isso: melhore apresentação antes de pensar só em preço. Fotos, descrições, diferenciais, avaliações e respostas a dúvidas devem deixar a compra mais fácil e segura.`
  };
  return map[channel.key]||`Como eu aplicaria isso: observe os padrões do canal, escolha os temas mais úteis para ${niche} e transforme em conteúdo claro, frequente e conectado a uma chamada de ação.`;
}
...
async function handleMarketIntel(req,res,{su,sk,gk,user}){
  ...
  if(action==='competitors'||action==='suggest_competitors'){
    ...
    let prompt=action==='suggest_competitors'
      ...
      :`Hoje é ${today}. Pesquise em tempo real movimentos, formatos, conteúdos, tendências e estratégias que profissionais, empresas e perfis relevantes do nicho abaixo estão usando ou intensificando nos últimos 30 dias. Gere exatamente ${channelFocus?'1 card para o canal clicado':'7 cards, um por canal de comunicação'}: ${channelFocus||channelList}.\n\n${marketCompanyText(company,profile,niche)}\nCanal que o usuário clicou primeiro: ${channelFocus||'não informado'}\n\nConsultas obrigatórias de descoberta pública, como se o usuário pesquisasse no Google e filtrasse por recente:\n${searchPlaybook}\n\nObjetivo da resposta:\n- Não repetir o checklist básico de estratégias consolidadas; ele já existe no card do usuário.\n- Encontrar padrões recentes, intensificações, formatos, temas, ganchos, CTAs, pautas e movimentos observáveis nos últimos 30 dias.\n- Usar exemplos reais pesquisados apenas como base para detectar padrões, sem listar cada concorrente individualmente.\n\nO checklist recente deve incluir, quando aplicável: novos formatos de conteúdo, Reels/vídeos/shorts, carrosséis ou posts salváveis, Stories/comunidade, ganchos, temas e pautas recentes, quadros recorrentes, educação, identificação, entretenimento, bastidores, prova social, collabs, podcasts/entrevistas/eventos, notícias e assuntos do momento, datas sazonais, comentários/salvamentos/compartilhamentos, CTAs, geração de leads, tráfego pago/remarketing e movimentos criativos.\n\nRegras obrigatórias:\n- Não cite nomes de concorrentes, marcas, perfis ou links no texto do card. Use a pesquisa só como base para descobrir padrões.\n- Evite dicas genéricas. Cada item precisa ser específico e aplicável ao nicho e ao canal.\n- Foque em movimentos recentes ou intensificados; não repita itens básicos como “ter bio clara” se isso não for movimento recente.\n- Não invente métrica, frequência ou resultado. Se a evidência for fraca, use linguagem honesta como “movimento aparente” ou “sinal recorrente”.\n- Não escreva textos crus como “Create an account or log in”, “instagram.com”, “&nbsp;”, nem despeje snippets.\n- Cada card deve ter title exatamente igual ao canal analisado, usando estes nomes: ${channelList}.\n- description deve começar com “Checklist — Movimentos recentes de ${niche} no [CANAL]” e trazer itens no formato “☐ Nome do movimento — explicação curta e prática.”\n- mark_strategy deve começar com “Como eu aplicaria isso:” e trazer um parágrafo estratégico com exemplos concretos de ganchos, quadros ou abordagens para o nicho.\n- Retorne SOMENTE JSON válido: {"cards":[{"competitor":"","title":"Instagram","description":"Checklist — Movimentos recentes de ${niche} no Instagram\\n☐ ...","mark_strategy":"Como eu aplicaria isso: ...","importance":0-5,"source":"Busca pública","source_url":"https://..."}]}.`;
    const strictChecklistRules=action==='competitors'?`\n\nReforço obrigatório de formato:\n- Cada description precisa ter de 6 a 10 itens úteis.\n- Cada item deve começar com o símbolo ☐ e seguir o formato “☐ Nome do movimento — explicação curta e prática.”\n- Não entregue parágrafos soltos no campo description. Não entregue lista de concorrentes. Não entregue fonte bruta.\n- O campo mark_strategy precisa começar exatamente com “Como eu aplicaria isso:” e trazer um parágrafo estratégico específico para ${niche}.\n- Se não conseguir confirmar movimentos recentes, ainda entregue checklist aplicável com movimentos aparentes do canal e diga isso de forma honesta em um dos itens.`:'';
    let obj={},sources=[],used=model;
