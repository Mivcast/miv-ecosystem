...
function cleanMarketCard(x){return {competitor:clean(x.competitor,120),channel:clean(x.channel||x.title,80),title:clean(x.title,180),description:clean(x.description,1800),mark_strategy:clean(x.mark_strategy||x.strategy,1200),importance:Math.max(0,Math.min(5,Number(x.importance)||3)),source:clean(x.source,140),source_url:clean(x.source_url||x.url,600)}}
...
function movementBullet(signal,channel,niche){
  ...
  if(/reel|short|vídeo|video|tiktok|youtube/.test(text))return `☐ Vídeos curtos de descoberta — transformar “${topic}” em conteúdo rápido, com gancho direto e uma ideia por vídeo.`;
  if(/dica|como|guia|passo|erro|mito|pergunta|dúvida|duvida/.test(text))return `☐ Conteúdo educativo por dúvida — usar perguntas reais do público para explicar “${topic}” com linguagem simples.`;
  if(/avalia|depoimento|cliente|resultado|antes|depois|case/.test(text))return `☐ Prova social e autoridade — reforçar confiança com experiências, bastidores, avaliações ou sinais de credibilidade.`;
  if(/campanha|evento|semana|mês|mes|data|lança|lanca|novo|novidade/.test(text))return `☐ Aproveitamento de assunto recente — conectar novidades, datas ou campanhas a uma pauta útil para o nicho.`;
  return `☐ Pauta atual com visão profissional — usar “${topic}” como gancho para educar, opinar e gerar conversa.`;
}
function movementFromSignals(channel,niche,signals){
  const usableSignals=(signals||[]).filter(x=>!isWeakSocialSignal(x)).slice(0,4);
  const base={
    instagram:[`☐ Posicionamento claro na bio — deixar evidente especialidade, localização, promessa principal e caminho para contato.`,`☐ Profissional como protagonista — usar rosto, voz e opinião para não depender só de artes institucionais.`,`☐ Reels educativos — responder dúvidas frequentes em 20 a 60 segundos para ganhar descoberta.`,`☐ Ganchos de identificação — começar por sintomas, desejos, medos ou situações que o público reconhece.`,`☐ Carrosséis salváveis — transformar sinais, erros, mitos e checklists em posts úteis para salvar.`,`☐ Stories de relacionamento — usar bastidores, enquetes e caixas de pergunta para gerar pauta e conversa.`,`☐ Prova social com cuidado — mostrar autoridade, rotina, eventos e depoimentos quando permitido pelo nicho.`,`☐ CTA simples — pedir para salvar, compartilhar, comentar dúvida ou chamar no WhatsApp sem vender em todo post.`],
    facebook:[`☐ Presença local consistente — publicar avisos, bastidores, provas e conteúdos de relacionamento.`,`☐ Comunidade e confiança — estimular comentários, mensagens e compartilhamentos em grupos ou página.`,`☐ Reaproveitamento inteligente — adaptar os melhores conteúdos do Instagram para públicos locais.`,`☐ Posts de utilidade — transformar dúvidas frequentes em orientações curtas para quem já acompanha a página.`,`☐ Provas e depoimentos — reforçar credibilidade com histórias, avaliações e bastidores permitidos.`,`☐ Datas e campanhas — aproveitar oportunidades sazonais com contexto do nicho.`,`☐ Conversa no inbox — usar CTAs para tirar dúvidas e encaminhar atendimento.`,`☐ Frequência simples — manter presença semanal mesmo quando o canal não é o principal.`],
    tiktok:[`☐ Ganchos rápidos — começar com sintomas, dúvidas, mitos ou situações cotidianas do público.`,`☐ Bastidores espontâneos — mostrar rotina, processo e opinião sem depender de artes formais.`,`☐ Quadros repetíveis — criar séries curtas para o público reconhecer e voltar.`,`☐ Mitos e verdades — corrigir crenças populares com linguagem simples.`,`☐ Respostas a comentários — transformar perguntas em novos vídeos.`,`☐ Conteúdo de identificação — fazer a pessoa pensar “isso acontece comigo”.`,`☐ Ritmo de teste — publicar variações do mesmo tema com ganchos diferentes.`,`☐ Reaproveitamento — levar os melhores vídeos para Reels e Shorts.`],
    'google-business':[`☐ Perfil atualizado — manter serviços, descrição, horários, fotos e links sem atrito.`,`☐ Prova social visível — estimular e responder avaliações com cuidado e linguagem do nicho.`,`☐ Posts rápidos de decisão — publicar novidades e diferenciais que ajudem quem está pesquisando agora.`,`☐ Fotos recentes — mostrar ambiente, equipe, produto ou resultado permitido para reduzir incerteza.`,`☐ Serviços bem nomeados — usar termos que o cliente realmente procura.`,`☐ Respostas humanizadas — responder avaliações sem parecer automático.`,`☐ Caminho para contato — deixar telefone, rota, WhatsApp ou agendamento fáceis.`,`☐ Sinais locais — reforçar bairro, cidade e área de atendimento.`],
    youtube:[`☐ Autoridade aprofundada — explicar temas complexos em vídeos mais completos e pesquisáveis.`,`☐ Cortes reaproveitáveis — transformar vídeos longos em shorts, Reels e carrosséis.`,`☐ Séries por dor do cliente — organizar playlists e quadros por dúvidas reais.`,`☐ Títulos pesquisáveis — usar perguntas que o público digitaria no Google ou YouTube.`,`☐ Explicação com exemplos — trocar teoria abstrata por situações do cotidiano.`,`☐ Conteúdo evergreen — criar vídeos que continuem úteis por meses.`,`☐ Entrevistas e collabs — trazer profissionais complementares para ampliar autoridade.`,`☐ CTA de relacionamento — convidar para comentário, inscrição, WhatsApp ou página de serviço.`],
    'site-landing-page':[`☐ Página por intenção — explicar serviços, diferenciais, provas e próximos passos sem depender só da rede social.`,`☐ Conteúdo pesquisável — usar artigos ou FAQs para dúvidas que o público busca no Google.`,`☐ Conversão clara — facilitar contato, WhatsApp, agendamento ou compra com botões visíveis.`,`☐ Promessa específica — deixar claro para quem é, qual dor resolve e por que escolher sua empresa.`,`☐ Provas e credenciais — mostrar diferenciais, cases, avaliações ou autoridade permitida.`,`☐ Perguntas frequentes — responder objeções antes do contato.`,`☐ Jornada simples — reduzir cliques e distrações até a ação principal.`,`☐ Páginas por serviço — criar conteúdos específicos em vez de uma página genérica para tudo.`],
  ...
  const recent=usableSignals.length?[...new Set(usableSignals.map(x=>movementBullet(x,channel,niche)))].slice(0,5):[`☐ Movimento recente aparente — monitorar temas atuais do nicho e transformar os melhores em conteúdo útil.`];
  return `Checklist — Posicionamento e Marketing de ${niche} no ${channel.label}\n${[...(base[channel.key]||[]),...recent].slice(0,9).join('\n')}`;
}
...
function channelStrategy(channel,niche){
  const map={
    instagram:`Sugestão estratégica para você: escolha 3 pilares para o Instagram de ${niche}: identificação, educação e autoridade. Grave Reels curtos com perguntas reais, transforme os melhores temas em carrosséis salváveis e use stories para bastidores, enquete e convite para conversa.`,
    facebook:`Sugestão estratégica para você: use o Facebook como presença local e relacionamento. Reaproveite conteúdos fortes, publique provas e avisos úteis, responda comentários e direcione conversas qualificadas para WhatsApp ou atendimento.`,
    tiktok:`Sugestão estratégica para você: crie uma série simples com ganchos de 20 a 40 segundos. Comece por mitos, erros comuns e situações do dia a dia, mantendo linguagem leve e repetição de quadro para ganhar reconhecimento.`,
    'google-business':`Sugestão estratégica para você: trate o Google Empresas como vitrine de decisão. Atualize fotos, serviços e descrição, publique novidades curtas e responda avaliações de forma cuidadosa para aumentar confiança local.`,
    youtube:`Sugestão estratégica para você: transforme dúvidas mais densas em vídeos explicativos e depois corte os melhores trechos para Reels, Shorts e carrosséis. Organize tudo por temas que o cliente realmente pesquisa.`,
    'site-landing-page':`Sugestão estratégica para você: crie páginas que respondam às dúvidas antes do contato. Use promessa clara, provas, FAQ e CTA visível, conectando os conteúdos das redes para uma página que converte melhor.`,
    marketplaces:`Sugestão estratégica para você: melhore apresentação antes de pensar só em preço. Fotos, descrições, diferenciais, avaliações e respostas a dúvidas devem deixar a compra mais fácil e segura.`
  };
  return map[channel.key]||`Sugestão estratégica para você: observe os padrões do canal, escolha os temas mais úteis para ${niche} e transforme em conteúdo claro, frequente e conectado a uma chamada de ação.`;
}
...
async function handleMarketIntel(req,res,{su,sk,gk,user}){
  ...
  if(action==='competitors'||action==='suggest_competitors'){
    ...
    const prompt=action==='suggest_competitors'
      ...
      :`Hoje é ${today}. Pesquise em tempo real estratégias de posicionamento, marketing e conteúdo usadas por profissionais, empresas e perfis relevantes do nicho abaixo. Gere exatamente ${channelFocus?'1 card para o canal clicado':'7 cards, um por canal de comunicação'}: ${channelFocus||channelList}.\n\n${marketCompanyText(company,profile,niche)}\nCanal que o usuário clicou primeiro: ${channelFocus||'não informado'}\n\nConsultas obrigatórias de descoberta pública, como se o usuário pesquisasse no Google e filtrasse por recente:\n${searchPlaybook}\n\nAnalise duas coisas ao mesmo tempo:\n1. Estratégias comuns e consolidadas entre os principais perfis desse nicho no canal.\n2. Movimentos, formatos, temas, tendências e estratégias que perfis relevantes estão usando ou intensificando nos últimos 30 dias.\n\nDepois una as duas análises em UM ÚNICO CHECKLIST RESUMIDO por canal. Não separe “estratégias comuns” de “estratégias recentes”.\n\nO checklist deve incluir, quando aplicável: posicionamento, bio/apresentação, autoridade, tipos de conteúdo, vídeos/Reels/shorts, carrosséis ou posts salváveis, stories/comunidade, ganchos, temas e pautas, quadros recorrentes, educação, identificação, entretenimento, bastidores, prova social, parcerias/collabs, podcasts/entrevistas/eventos, notícias e assuntos do momento, datas sazonais, comentários/salvamentos/compartilhamentos, CTAs, geração de leads, tráfego pago/remarketing e movimentos criativos.\n\nRegras obrigatórias:\n- Não cite nomes de concorrentes, marcas, perfis ou links no texto do card. Use a pesquisa só como base para descobrir padrões.\n- Evite dicas genéricas. Cada item precisa ser específico e aplicável ao nicho e ao canal.\n- Misture estratégias consolidadas e movimentos recentes no mesmo checklist.\n- Não invente métrica, frequência ou resultado. Se a evidência for fraca, use linguagem honesta como “movimento aparente” ou “sinal recorrente”.\n- Não escreva textos crus como “Create an account or log in”, “instagram.com”, “&nbsp;”, nem despeje snippets.\n- Cada card deve ter title exatamente igual ao canal analisado, usando estes nomes: ${channelList}.\n- description deve começar com “Checklist — Posicionamento e Marketing de ${niche} no [CANAL]” e trazer itens no formato “☐ Nome da estratégia — explicação curta e prática.”\n- mark_strategy deve começar com “Como eu aplicaria isso:” e trazer um parágrafo estratégico com exemplos concretos de ganchos, quadros ou abordagens para o nicho.\n- Retorne SOMENTE JSON válido: {"cards":[{"competitor":"","title":"Instagram","description":"Checklist — Posicionamento e Marketing de ${niche} no Instagram\\n☐ ...","mark_strategy":"Como eu aplicaria isso: ...","importance":0-5,"source":"Busca pública","source_url":"https://..."}]}.`;
    const strictChecklistRules=action==='competitors'?`\n\nReforço obrigatório de formato:\n- Cada description precisa ter de 8 a 12 itens quando houver informação suficiente, e no mínimo 6 itens úteis.\n- Cada item deve começar com o símbolo ☐ e seguir o formato “☐ Nome da estratégia — explicação curta e prática.”\n- Não entregue parágrafos soltos no campo description. Não entregue lista de concorrentes. Não entregue fonte bruta.\n- O campo mark_strategy precisa começar exatamente com “Como eu aplicaria isso:” e trazer um parágrafo estratégico específico para ${niche}.\n- Se não conseguir confirmar movimentos recentes, ainda entregue checklist aplicável misturando padrões consolidados do nicho com movimentos aparentes e diga isso de forma honesta em um dos itens.`:'';
    let obj={},sources=[],used=model;
    if(gk)try{({obj,sources,model:used}=await generateMarketIntel(gk,model,prompt+strictChecklistRules,action==='competitors'?9000:5200))}catch(e){console.warn('[MARKET INTEL competitors fallback]',e.message)}
    ...
    const badCard=card=>{
      ...
      if(!/^checklist\s+—/i.test(String(card.description||'').trim())||!String(card.description||'').includes('☐'))return true;
      if(!/^como eu aplicaria isso:/i.test(String(card.mark_strategy||'').trim()))return true;
